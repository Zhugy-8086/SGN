...
//
// 数学依据：fixes_相关修复/msint_int8_pair_grad_carrier_2026_08_31.md §六/§七
...
//
//   ⚠️ coarse 语义声明（2026-09-06 审查 E.5，见 fixes_相关修复/
...
//   真实梯度消费**；若需无偏粗估计，消费端加 **+127.5·Δ·Sw** 补偿（Δ = p.scale）。
//   ⚠️ 常数修正（2026-09-08 E.5 判读，docs/问题追踪/E5判读_coarse截断偏差与
//   补偿常数_2026_09_08.md）：原写 +128 系工程近似，**非严格无偏**。sympy 符号
//   精确解 E[l''] = (M−1)/2 = 255/2 ⇒ **c* = 127.5**（M=256，通项 (M−1)/2）；
//   +128 残留 **+0.5Δ**（相对原偏差 127.5Δ 的 0.392%）。两常数在点积层的差异
//   被 l'' 波动淹没（实测 SE≈3.7e4 vs 差异 4.9e2，不可分辨），故工程上 +128 亦可
//   接受；追求期望严格无偏则用 127.5（= 255·Sw/2，需 Sw 偶数或走定点）。
//   另注：多周期（σ/M ≥ 1，Q16 实测 σ/M ≈ 36）下 c* ∈ [127.35, 127.55] 鲁棒；
//   **亚周期（σ/M < 1）真实分布依赖**（实测偏离至 125.2），须按分布标定，勿硬编码。
//
// 落地约束（#32 入口瓶颈定性）：
