...
static void test_dot8_dot4() {
    ...
    // 大 K int32 溢出防护（2026-09-05 dot8 审查）：满幅对抗跨块界。
    // 修复前 dot8_vnni K>526,296 回绕、dot8_avx2 K>1,052,631 回绕（实测）。
    // 262143/262144/262145 = 2^18 块界 ±1；526296/1048576 = 旧溢出阈值域。
    for (size_t K : {262143u, 262144u, 262145u, 526296u, 1048576u}) {
        for (int pos = 0; pos <= 1; ++pos) {  // 满幅 255×(−128/127) 两向
            std::vector<uint8_t> u(K, 255);
            std::vector<int8_t>  s(K, pos ? 127 : -128);
            CHECK(dot8(u.data(), s.data(), K) == dot8_scalar(u.data(), s.data(), K),
                  "dot8 large-K full-scale mismatch");
            CHECK(dot4(u.data(), s.data(), K) == dot4_scalar(u.data(), s.data(), K),
                  "dot4 large-K full-scale mismatch");
        }
    }
