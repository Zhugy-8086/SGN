...
//
// 背景：SIMD 原语域拆分设计（平台无关接口契约 + per-ISA 实现 + 运行时 CPUID 调度）
//
...
//   bit-exact（对**全部 K** 成立，含满幅对抗——2026-09-05 dot8 审查后各 SIMD 后端
//   以 2^18 分块 + 块间 int64 累加防护 int32 lane 溢出，K ≤ 2^18 为无分块快路径；
//   见 内部档案）。
...
// （解包在 4 位路径由调用方负责，故 dot4 与 dot8 同实现而非自带 4 位特化）。
// bit-exact（全 K，同 dot8 大 K 防护口径——各后端 dot4 委托/共享 dot8 主体）。
...
// 由各后端实现（scalar / avx2 / avxvnni / avx512vnni），bit-exact（全 K，
// 同 dot8 大 K 防护口径：2^18 分块 + 块间 int64 累加）。
