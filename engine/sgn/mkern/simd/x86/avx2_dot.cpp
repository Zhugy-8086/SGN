...
//
// 背景：SIMD 原语域拆分设计（内部） + 综合执行计划
//   （simd_dot8_avx2中间路径设计档_2026_09_02.md）。本文件为 avx2.cpp 按原语域拆分的
//   点积域：dot16（迁移）+ dot8/dot4 AVX2 中间档（新增，见设计档 §一）。
...
// 单块上限 2^18（同 avxvnni.cpp kAccSafeChunk）：merge 后 lane ≤ 5.348e8 < 2^31
// （2026-09-05 dot8 审查，见 内部档案）。
constexpr size_t kAccSafeChunk = size_t(1) << 18;

inline __attribute__((always_inline)) int64_t dot8_avx2_block(const uint8_t* a, const int8_t* b, size_t K) {
    ...
    // （2026-09-05 审查注：上式仅覆盖 K=65536 基准上界；本块上限 2^18 由
    // kAccSafeChunk 保证，超出部分由 wrapper 块间 int64 累加承接。）
...
int64_t dot8_avx2(const uint8_t* a, const int8_t* b, size_t K) {
    // 大 K 溢出防护（2026-09-05 dot8 审查，沙箱 52/52 对抗 PASS）：满幅对抗下
    // K>1,052,631 时 lo0+lo1 int32 合并回绕（实测 K=2,359,296 偏差 ~6.9e10）。
    // 按 2^18 分块、块间 int64 累加——整数加法可结合，与标量锚点全 K bit-exact。
    // 快路径（同 dot8_vnni）：K ≤ 块上限直接返回块体，现役域与旧版性能持平。
    if (K <= kAccSafeChunk) return dot8_avx2_block(a, b, K);
    int64_t total = 0;
    for (size_t done = 0; done < K; done += kAccSafeChunk) {
        const size_t n = std::min(kAccSafeChunk, K - done);
        total += dot8_avx2_block(a + done, b + done, n);
    }
    return total;
}
...
inline __attribute__((always_inline)) int64_t dot4_packed_avx2_block(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
    __m256i lo0 = _mm256_setzero_si256(), hi0 = _mm256_setzero_si256();
    __m256i lo1 = _mm256_setzero_si256(), hi1 = _mm256_setzero_si256();
    size_t i = 0;  // 元素索引
...
int64_t dot4_packed_avx2(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
    // 大 K 溢出防护（2026-09-05 dot8 审查）：a∈[0,15] 幅度小（阈值 K ≈ 2.9 亿
    // 才危险），契约上与 dot8 统一 2^18 分块 + 块间 int64 累加（全 K bit-exact）。
    // done 恒为偶数（2^18 与批宽均为偶数），done>>1 打包指针推进无舍入风险；
    // 奇数 K 尾部由 block 内标量 tail 处理。快路径（同 dot8_avx2）。
    if (K <= kAccSafeChunk) return dot4_packed_avx2_block(a_packed, b_packed, K);
    int64_t total = 0;
    for (size_t done = 0; done < K; done += kAccSafeChunk) {
        const size_t n = std::min(kAccSafeChunk, K - done);
        total += dot4_packed_avx2_block(a_packed + (done >> 1),
                                        b_packed + (done >> 1), n);
    }
    return total;
}
