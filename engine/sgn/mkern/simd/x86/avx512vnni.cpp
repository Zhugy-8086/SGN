...
inline __attribute__((always_inline)) int64_t dot8_avx512vnni_block(const uint8_t* a, const int8_t* b, size_t K) {
...
int64_t dot8_avx512vnni(const uint8_t* a, const int8_t* b, size_t K) {
    // 大 K 溢出防护（2026-09-05 dot8 审查，同 256 位 vnni/avx2 模式）：按 2^18
    // 分块、块间 int64 累加——整数加法可结合，与标量锚点全 K bit-exact。
    // 快路径（同 dot8_vnni）：K ≤ 块上限直接返回块体。512 位路径待 EPYC 复验。
    if (K <= kAccSafeChunk) return dot8_avx512vnni_block(a, b, K);
    int64_t total = 0;
    for (size_t done = 0; done < K; done += kAccSafeChunk) {
        const size_t n = std::min(kAccSafeChunk, K - done);
        total += dot8_avx512vnni_block(a + done, b + done, n);
    }
    return total;
}
...
int64_t dot4_avx512vnni(const uint8_t* u8, const int8_t* s8, size_t K) {
    ...
    // 直接委托 dot8_avx512vnni（同 avxvnni/avx2_dot 委托纪律），自动获得
    // 大 K int32 溢出防护（2026-09-05 dot8 审查）。
    return dot8_avx512vnni(u8, s8, K);
}
...
inline __attribute__((always_inline)) int64_t dot4_packed_avx512vnni_block(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
...
int64_t dot4_packed_avx512vnni(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
    ...
    // 奇数 K 尾部由 block 内标量 tail 处理。快路径（同 dot8_avx512vnni）。
    if (K <= kAccSafeChunk) return dot4_packed_avx512vnni_block(a_packed, b_packed, K);
    int64_t total = 0;
    for (size_t done = 0; done < K; done += kAccSafeChunk) {
        const size_t n = std::min(kAccSafeChunk, K - done);
        total += dot4_packed_avx512vnni_block(a_packed + (done >> 1),
                                              b_packed + (done >> 1), n);
    }
    return total;
}
