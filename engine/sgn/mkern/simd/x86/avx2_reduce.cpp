...
//   （simd_dot8_avx2中间路径设计档_2026_09_02.md §二.5 拆分）。本文件为 avx2.cpp
...
//
// 编译：CMake 对本文件加 -mavx2 -ffp-contract=off（R1 修订 2026-09-03）。本实现只用
// add/mul（不用 _mm256_fmadd），且必须禁 FMA 收缩：若编译器把标量尾部 sum += d*d
// 收缩成单舍入 vfmadd（-mfma/-march=native 下会发生），即破坏与 scalar 锚点的逐位一致。
// 显式 _mm256_fmadd intrinsic 不受 -ffp-contract=off 影响，未来加 FMA 快速路径时仍可用。
