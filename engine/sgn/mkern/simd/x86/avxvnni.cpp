...
// 单块上限 2^18：块内 int32 lane 累加最坏（满幅 255×−128，每 lane 每批
// +4×32640 = 130,560）4 acc 合并 ≤ (2^18/128)×130,560×4 = 1,069,547,520
// < 2^31，恒不溢出（2026-09-05 dot8 审查，见 内部档案）。
constexpr size_t kAccSafeChunk = size_t(1) << 18;

inline __attribute__((always_inline)) int64_t dot8_vnni_block(const uint8_t* a, const int8_t* b, size_t K) {
...
int64_t dot8_vnni(const uint8_t* a, const int8_t* b, size_t K) {
    // 大 K 溢出防护（2026-09-05 dot8 审查，沙箱 52/52 对抗 PASS）：
    // int32 lane 整程累加在满幅对抗输入下 K>526,296 静默回绕（实测 K=600K
    // 偏差 ~343 亿）。按 2^18 分块、块间 int64 累加——整数加法可结合，与标量
    // 锚点全 K bit-exact。
    // 快路径：K ≤ 块上限直接返回块体（block 强制内联，与旧版指令序列同构 +
    // 1 个可预测分支——交错 A/B 实测消除 wrapper 循环前导的 ~1ns/调用固定
    // 开销，现役域 K=1024/4096/65536 性能与旧版持平或略优）。
    if (K <= kAccSafeChunk) return dot8_vnni_block(a, b, K);
    int64_t total = 0;
    for (size_t done = 0; done < K; done += kAccSafeChunk) {
        const size_t n = std::min(kAccSafeChunk, K - done);
        total += dot8_vnni_block(a + done, b + done, n);
    }
    return total;
}
...
int64_t dot4_vnni(const uint8_t* u8, const int8_t* s8, size_t K) {
    ...
    // 无解包指令；语义等价于逐元素 u8×s8 求和，bit-exact）。与 dot8 完全同载体
    // ——直接委托 dot8_vnni（对齐 avx2_dot.cpp dot4_avx2 = dot8_avx2 既有纪律），
    // 自动获得大 K int32 溢出防护（2026-09-05 dot8 审查）。
    return dot8_vnni(u8, s8, K);
}
...
inline __attribute__((always_inline)) int64_t dot4_packed_vnni_block(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
...
int64_t dot4_packed_vnni(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
    // 大 K 溢出防护（2026-09-05 dot8 审查）：a∈[0,15] 幅度小（merge lane 每块
    // ≤1.6e7，阈值 K ≈ 1.4 亿才危险），契约上与 dot8 统一 2^18 分块 + 块间
    // int64 累加（全 K bit-exact）。done 恒为偶数（2^18 与批宽均为偶数），
    // done>>1 打包指针推进无舍入风险；奇数 K 尾部由 block 内标量 tail 处理。
    // 快路径（同 dot8_vnni）：K ≤ 块上限直接返回块体。
    if (K <= kAccSafeChunk) return dot4_packed_vnni_block(a_packed, b_packed, K);
    int64_t total = 0;
    for (size_t done = 0; done < K; done += kAccSafeChunk) {
        const size_t n = std::min(kAccSafeChunk, K - done);
        total += dot4_packed_vnni_block(a_packed + (done >> 1),
                                        b_packed + (done >> 1), n);
    }
    return total;
}
