...
# include 路径指向 engine/sgn/ 根（本脚本位于 engine/sgn/mkern/simd/，../.. = engine/sgn），
# 因源文件用 #include "mkern/simd/simd_api.h"。
BASE_FLAGS="-O3 -std=c++23 -I../.. -ffp-contract=off"
if [ "$UBSAN" = "1" ]; then
    BASE_FLAGS="-O1 -std=c++23 -I../.. -ffp-contract=off -fsanitize=undefined -fno-sanitize-recover=all"
fi
...
FILE_FLAGS[x86/avx2_reduce.cpp]="-mavx2 -ffp-contract=off"
FILE_FLAGS[x86/avxvnni.cpp]="-mavx2 -mavxvnni"
FILE_FLAGS[x86/ssse3.cpp]="-mssse3 -mavx2"
FILE_FLAGS[x86/avx512.cpp]="-mavx512f -mavx512bw -mavx512vl -ffp-contract=off"
FILE_FLAGS[x86/avx512vnni.cpp]="-mavx512f -mavx512bw -mavx512vl -mavx512vnni -ffp-contract=off"
