...
int64_t dot4_packed_scalar(const uint8_t*, const int8_t*, size_t);  // R2 4 位打包点积
...
int64_t dot4_packed_vnni(const uint8_t*, const int8_t*, size_t);  // R2 4 位打包点积
...
int64_t dot4_packed_avx2(const uint8_t*, const int8_t*, size_t);  // R2 4 位打包点积
...
int64_t dot4_packed_avx512vnni(const uint8_t*, const int8_t*, size_t);  // R2 4 位打包点积
...
const SimdBackend& simd_backend() noexcept {
    ...
    static const SimdBackend s = [] {
        ...
        if (forced && (std::strcmp(forced, "scalar") == 0 ||
            ...
            b.dot8             = dot8_scalar;
            b.dot4             = dot4_scalar;
            b.dot4_packed      = dot4_packed_scalar;
        ...
        if (caps.avx_vnni) {
            b.dot8 = dot8_vnni;
            b.dot4 = dot4_vnni;
            b.dot4_packed = dot4_packed_vnni;  // R2 4 位打包点积
        }
        if (caps.avx2) {
            b.dot16           = dot16_avx2;
            ...
            if (!caps.avx_vnni && !caps.avx512_vnni) {
                b.dot8 = dot8_avx2;
                b.dot4 = dot4_avx2;
                b.dot4_packed = dot4_packed_avx2;  // R2 4 位打包点积
            }
        }
        if (caps.ssse3) {
        ...
        if (caps.avx512_vnni && caps.avx512_bw) {
            ...
            b.dot4_packed = dot4_packed_avx512vnni;  // R2 4 位打包点积
        }
...
// 原语精度档位查询（R1 可复现化 2026-09-03 更新）：后端无关。
//   - 整型/字节原语 → kBitExact（结果与后端无关）
//   - sum 系 → kBitExact（R1 统一固定 8 路 + 固定归约树 + 固定尾，scalar/avx2/avx512
//     逐位一致，见 simd_api.h 契约）
//   - accum_f32 → kBitExact（逐元素独立加法，无跨元素归约，天然逐位一致）
...
int64_t dot4_packed(const uint8_t* a_packed, const int8_t* b_packed, size_t K) {
    return simd_backend().dot4_packed(a_packed, b_packed, K);
}
