...
def extract_layers(intensity: List[int], layer_max: int = 4, d: int = 16, strategy=None) -> Tuple[List[int], int]:
    ...
    try:
        from sgn_input import DefaultLayerExtractor
...
def gate_match(high_in: int, low_in: int, high_tpl: int, low_tpl: int,
               d: int = None, high_thresh: int = 60, low_thresh: int = 50) -> Tuple[bool, int, int]:
    """算法 C：门控匹配（高层 AND 低层）

    只有高层特征匹配且低层特征匹配时，才允许通过。
    ...
    # 高层匹配度（边缘重叠率）：交集/并集 * 100
    high_union = popcount(high_in | high_tpl)
    high_match = (popcount(high_in & high_tpl) * 100) // max(1, high_union)

    # 低层匹配度（像素重叠率）：交集/并集 * 100
    low_union = popcount(low_in | low_tpl)
    low_match = (popcount(low_in & low_tpl) * 100) // max(1, low_union)

    # 门控：高层 AND 低层
    passed = (high_match >= high_thresh) and (low_match >= low_thresh)

    return passed, high_match, low_match
