...
def create_neuron(nid, d=16):
    ...
    return {
        ...
        "base_index": base_dc.index,   # v5.1.5: 纯整数存储（双写）
        "lock": 0,
        "enc_r": 0,
        "enc_b": DiscreteCoordinate(0, base_dc.level),  # 初始为 0，同层级
        "enc_b_index": 0,              # v5.1.5: 纯整数存储（双写）
        "L": False,
        # v5.1 多层神经元新增字段
        "gate": DiscreteCoordinate(100, 2),        # 门控强度 (0~100)
        "gate_index": 100,             # v5.1.5: 纯整数存储（双写）
...
class SGNCore:
    ...
    def _extract_gate_features(self, layers, layer_count, intensity):
        ...
        from .layers import extract_edge_map
        input_mask = layers[0]
        ...
        for nid in winners:
            ...
            if verified:
                ...
                if new_index <= sat_t.index:
                    ...
                    n["base_index"] = new_index  # v5.1.5 双写
                else:
                    n["base"] = sat_t  # 饱和
                    n["base_index"] = sat_t.index  # v5.1.5 双写
                # 2. 鼓励触发（短期脉冲）
                n["enc_r"] = enc_cnt
                n["enc_b"] = enc_t
                n["enc_b_index"] = enc_t.index  # v5.1.5 双写
                ...
                if CONFIG.get("ENABLE_SOFT_GATE", False):
                    if n["specialization"] is None:
                        ...
                        n["gate_index"] = new_gate  # v5.1.5 双写
            else:
                ...
                if new_index > min_t.index:
                    ...
                    n["base"] = min_t  # 不低于最小值
                    n["base_index"] = min_t.index  # v5.1.5 双写
                n["lock"] += 1
    ...
    def _update_atoms(self, label, mask):
        """学习成功时更新按类别原子字典"""
        from .layers import extract_patch_vector
