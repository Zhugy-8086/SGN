...
import os
import re
from sgn_config import CONFIG  # 统一导入，避免函数内重复
...
def gen_samples(label, count, noise_prob=0.15):
    ...
    确保新旧代码行为一致，避免双轨制分叉。
    """
    from sgn_input import PatternInputSource, DefaultCompositeNoise
    from sgn_config import PATTERNS
...
from sgn_layers import popcount, match_bits, extract_layers, combine_layers, classify_sample
...
def run_cross_validation(source, max_step):
    ...
    4. 记录结果，销毁网络
    """
    from sgn_config import LABELS, PATTERNS, CONFIG
    from sgn_input import PatternInputSource, DefaultCompositeNoise
    from sgn_core import SGNCore
    import random
    from sgn_utils import log_print, C
