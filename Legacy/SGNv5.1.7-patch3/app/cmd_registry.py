...
def _cmd_stats(core, **kwargs):
    """[s] 统计信息"""
    from app.visual import do_stats
    do_stats(core)


def _cmd_gauge(core, **kwargs):
    """[g] 仪表盘"""
    from app.visual import do_gauge
    do_gauge(core)


def _cmd_visualize(core, **kwargs):
    """[u] 模板可视化"""
    from app.visual import do_visualize
    do_visualize(core)


def _cmd_heatmap(core, **kwargs):
    """[m] 热力图"""
    from app.visual import do_heatmap
    do_heatmap(core)


def _cmd_confusion(core, **kwargs):
    """[c] 混淆矩阵"""
    from app.test import do_confusion
    test_samples = kwargs.get("test_samples") or kwargs.get("samples")
...
def _cmd_noise_test(core, **kwargs):
    """[n] 噪声测试"""
    from app.test import do_noise_test
    test_samples = kwargs.get("test_samples") or kwargs.get("samples")
...
def _cmd_cross_validate(core=None, **kwargs):
    ...
    from engine.utils import run_cross_validation, C
...
def _cmd_bug_check(core=None, **kwargs):
    """[z] 修复验证"""
    try:
        from tests.test_training_fixes import run
        run()
    except ImportError as e:
        from engine.utils import C
        print(f"  {C.err('✗')} 测试模块加载失败: {e}")
    except Exception as e:
        from engine.utils import C
        print(f"  {C.err('✗')} 测试运行异常: {e}")
...
def register_all_commands():
    ...
    defaults = [
        ...
        Command("f",  "函数工厂", "系统",  _cmd_factory,     requires_trained=False, order=8),
        ...
        Command("z",  "修复验证", "系统",  _cmd_bug_check,   requires_trained=False, order=1),
        Command("w",  "成功率",  "系统",  _cmd_why_accuracy, requires_trained=False, order=2),
        Command("o",  "保存",    "系统",  _cmd_save,        requires_trained=False, order=3),
        Command("d",  "加载",    "系统",  _cmd_load,        requires_trained=False, order=4),
        Command("k",  "模式",    "系统",  _cmd_mode_switch, requires_trained=False, order=5),
        Command("h",  "帮助",    "系统",  _cmd_help,        requires_trained=False, order=6),
        Command("b",  "返回面板", "系统",  _cmd_back,       requires_trained=False, order=7),
