...
def _classify_single(core, intensity, d=None) -> Tuple[str, int]:
    ...
    return classify_with_mode(core, intensity, d)[:2]
...
class AccuracyMetric(Metric):
    ...
    def compute(self, core, test_samples: List[Tuple], **kwargs) -> Dict[str, Any]:
        ...
        for intensity, label in test_samples:
            ...
            match, pred_lb, best_s, _ = classify_pass(core, intensity, label)
            if match:
                correct += 1
...
class NoiseRobustnessMetric(Metric):
    ...
    def compute(self, core, test_samples: List[Tuple] = None, **kwargs) -> Dict[str, Any]:
        from engine.config import CONFIG
        from engine.input import DefaultCompositeNoise, GaussianNoise, SaltPepperNoise, BlockNoise
        ...
        if test_samples is not None:
            ...
            for flip in self.flip_probs:
                ...
                for intensity, label in test_samples:
                    ...
                    if self.noise_type == "gaussian":
                        ...
                        gs = int(getattr(core, 'grid_size', CONFIG.get("D", 64) ** 0.5))
                    ...
                    from app.classify_utils import classify_pass
                    match, _, _, _ = classify_pass(core, noisy, label)
                    if match:
                        correct += 1
                    total += 1
            ...
            for flip in self.flip_probs:
                ...
                for intensity, label in samples:
                    match, _, _, _ = classify_pass(core, intensity, label)
                    if match:
                        correct += 1
                    total += 1
                pct = (correct / total * 100) if total else 0
