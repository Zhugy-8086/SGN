...
```
============================================================
  SGN-Lite v5.1.7 (Level Scheduler + Adaptive Silence + L1 Decision Layer + Modular Engine)
============================================================
...
| L1_DECISION_LAYER | True | -- | L1 决策层/自组织分类 (v5.1.6) |
| L1_BUCKET_ENABLED | True | -- | L1 图桶化防串位 (v5.1.6) |
| L1_PEER_COMPARE_ENABLED | True | -- | L1 同级比较聚类 (v5.1.6) |
...
v5.1.6 将分类从**外部分类器代码遍历标签**改为**神经元输出结果自组织涌现**：

**核心思想**：L1 神经元不输出标签，输出"判断模式向量"。输出相似的 L1 神经元自然聚成一类，聚类标签=涌现分类——不是被代码分配的，而是涌现的。

**四项机制**：
1. **L1 图模式桶化与链条化（3.3）**：按标签把 winner 分到不同子图（桶），防数据串位；桶内 winner 按模板相似度串链，保留特征演化路径
2. **L1 决策层（3.4）**：L1 输出判断模式向量（不是标签），代表对输入的判断倾向
3. **L1 同级比较与自组织聚类（3.4）**：与 L0 同级比较对称——L0 比较输入模板，L1 比较输出模式；输出相似的归为同一聚类，聚类标签=涌现分类
4. **L1 回馈图模式（3.4）**：判断正确时 L1 调整图模式（中间层），简化的反向传播

**补丁 P2（纠错机制）**：簇置信度 + 周期性重组 + 负反馈拆簇，三层纠错避免错误聚类固化。

**向后兼容**：`L1_DECISION_LAYER` 关闭或无 `cluster_id` 时回退到遍历标签匹配。

> **涌现分类需训练积累**：随机样本+少量训练时 `verified` 全 False 是正常的，`cluster_id` 全为 `"?"` 是预期行为，真实标签会随训练积累涌现。

核心配置：
- `L1_BUCKET_ENABLED` / `L1_BUCKET_BY` / `L1_CHAIN_ENABLED` / `L1_CHAIN_SIMILARITY`: 桶化与链条化
- `L1_DECISION_LAYER` / `L1_OUTPUT_PATTERN_DIM`: 决策层开关与输出维度
- `L1_PEER_COMPARE_ENABLED` / `L1_CLUSTER_SIMILARITY`: 同级比较与聚类阈值
- `L1_FEEDBACK_TO_GRAPH` / `L1_FEEDBACK_STRENGTH`: 回馈图模式
- `L1_CLUSTER_CONFIDENCE_THRESH` / `L1_CLUSTER_REORG_INTERVAL`: P2 纠错机制

详见 [`docs/l1-decision-layer-v5.1.6.md`](l1-decision-layer-v5.1.6.md)。
...
> **版本公开说明**: v5.1.6 为内部开发版本，未对外公开发布。

## 当前版本声明 (先验机状态)

> **SGN-Lite v5.1.7 目前处于"先验机"状态——即正在完善中的原型机，尚未达到完整原型机阶段。**
>
...
>
> **当前已实现**: 记忆层面的优化代码 (图模式层级记忆、模板合并、竞争学习机制、Level 调度器、自适应随机静默机制、L1 决策层与自组织分类、模块化引擎架构)
>
> **尚未完善**:
