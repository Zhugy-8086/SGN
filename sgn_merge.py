...
from sgn_graph import GraphNode, DynamicGraph
...
def _merge_cluster_nodes(
    ...
    for dim_idx in range(max_len):
        ...
        merged_feat.append(DiscreteCoordinate(median_idx, target_lvl))
    ...
    if len(merged_feat) >= 2:
        merged_feat[0] = DiscreteCoordinate(avg_r, 2)
        merged_feat[1] = DiscreteCoordinate(avg_c, 2)
    else:
        merged_feat = [DiscreteCoordinate(avg_r, 2), DiscreteCoordinate(avg_c, 2)] + merged_feat

    # 汇总激活
    total_act = sum(n.activation for n in cluster)

    # 合并layer_history
    all_history = []
    for n in cluster:
        all_history.extend(n.layer_history)
    all_history = list(set(all_history))

    # 合并source_neurons
    all_neurons = []
    for n in cluster:
        all_neurons.extend(n.source_neurons)
    ...
    return GraphNode(
        ...
        last_updated=step,
        label=label,
        layer_history=all_history or [layer],
        source_neurons=all_neurons
    )
