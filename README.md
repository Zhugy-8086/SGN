...
Sparse Graph Network Lite —— 轻量级整数化竞争学习识别系统

## 简介

SGN-Lite 是一个**核心识别路径无浮点、无反向传播**的轻量级识别引擎。核心机制为整数化竞争学习 + 模板匹配，适用于算法验证与概念原型开发。

**核心特性**:
- **核心引擎整数运算**（DiscreteCoordinate 离散坐标体系，当前内嵌于 core，未来随 HC 库统一重构）
- 竞争学习 + 赫布增强/削弱
- 双图叠加门控识别（v4.4）
- 图模式层级记忆（v5.0）
- 多层竞争扩展（v5.1.3）
- GUI 函数工厂（手绘像素图形、实时变换、训练测试）
...
- **整数化竞争学习**：DiscreteCoordinate 离散坐标体系（level 0~3），浮点一次性投影后全程整数运算
- **模板匹配**：XOR 距离 + 响应速度排序 + Top-K 竞争
- **图模式（v5.0）**：神经元 winner 投影为图节点 → 赫布合并 → 一致性过滤。图与神经元为**单向流水线**，图输出特征向量供后续使用
- **多层竞争扩展（v5.1.3，默认关闭）**：Layer 0（原始掩码竞争）→ 图特征提取（10维向量）→ Layer 1（图特征竞争）。L0 与 L1 **同构**（均为竞争+赫布），非层级抽象
- **输入**：4×4/8×8 字符、矢量图形（line/circle/sine/arch/leaf）、噪声模型（高斯/椒盐/块遮挡）
- **GUI**：pygame 函数工厂，支持手绘、撤销/重做、训练集管理、PNG 导出

### 当前限制

| 限制 | 说明 |
|------|------|
| 无颜色识别 | 当前仅支持灰度/二值输入，颜色需多通道离散化，core 匹配逻辑未 ready |
| 无时间维度 | 时间变量当前仅为计步器，真正的时间处理依赖 HC 库 |
| 图与神经元独立管线 | 图从神经元单向投影生成，无双向反馈。深度耦合需等上层神经元架构完成后统一接口 |
| 神经元输出单一 | 当前仅输出 winner ID，未来上层架构需要结构化信号（位置/置信/关系） |
| 数值设定为测试优化 | 神经元数量、模板上限等参数为验证框架配置，非生产环境设定 |

### 计划中（尚未实现）

| 项目 | 状态 | 说明 |
|------|------|------|
| **颜色识别** | 计划中 | 视觉通用化的关键路径。需重构输入层为多通道 + core 匹配逻辑支持多通道离散坐标。当前 core 机制不完善，暂无法支撑 |
| **HC 时间库** | 制作中 | 将统一 level/整数化基础设施（当前内嵌于 core 的 DiscreteCoordinate 未来迁移至此）。接入时 core 需配合重写 |
| **上层神经元架构** | 设计中 | 非 v5.1.3 的 L1。真正的组合逻辑层，输出结构化信号，图模式将与其深度耦合。当前底层神经元（winner ID 输出）不足以支撑 |

### 为什么现在不做

- **颜色识别**：core 的匹配逻辑和输入表示需大幅改动，在 core 机制定型前接入会导致后续重构成本倍增
- **HC 库**：level 系统当前内嵌于 core，是临时方案。等 HC 库成熟后统一迁移，避免 core 里做两遍
- **上层架构**：需要底层神经元先输出结构化信号（而非单一 ID），当前底层设计未定，强兼容后续必改
- **数值上限固定**：当前为测试框架，无需复杂动态管理。提前实现会限制后续机制设计的自由度
...
```
============================================================
  SGN-Lite v5.1.3 Python PC平台验证脚本
============================================================

  模块结构:
    核心层 (Core):
      sgn_core.py      核心引擎(整数化) - 当前内嵌 level 系统
    扩展层 (Extension):
...
```
核心层 (Core)
  sgn_core.py          核心引擎（竞争/校验/学习/模板合并/多层竞争扩展 v5.1.3）
                       DiscreteCoordinate level 系统当前内嵌于此，未来随 HC 库重构
...
| 配置 | 默认值 | 可调范围 | 说明 |
|------|--------|---------|------|
| MAX_NEURONS | 256 | 1 ~ 4096 | 神经元数量上限（测试框架配置） |
...
| ENABLE_MULTI_LAYER_NEURON | False | -- | 多层竞争扩展 (v5.1.3) |
...
> **注意**: 图模式当前与神经元为**单向流水线**。图从神经元投影生成，无反馈回路。深度双向耦合需等待上层神经元架构完成后统一实现。

---

## 多层竞争扩展 (v5.1.3)

v5.1.3 在神经元之上新增 Layer 1 竞争层，通过图中间层汇总器连接两层神经元。
...
**数据流**:
- Layer 0 神经元竞争（基础特征匹配：边缘/局部模式）
- 获胜者投影为图节点 → 图赫布融合
- 提取图结构特征（10维固定向量）+ L0 激活 ID 列表
- Layer 1 神经元竞争（图特征向量相似度匹配）
- 两层各自独立赫布学习

> **重要**: Layer 0 与 Layer 1 **同构**，均为竞争+赫布学习，区别仅在于输入格式（原始掩码 vs 图特征向量）。这不是"层级抽象"或"概念判断"，而是同一机制在两种输入上的重复应用。
...
- v5.1.3: 多层竞争扩展 — Layer 0/1 双层竞争、图中间层汇总器、软门控、标签专精分片、13个Bug修复

---

## 当前版本声明 (先验机状态)

> **SGN-Lite v5.1.3 目前处于一个神经网络，但没做完，不是一个完整的神经网络原型机，是一个正在完善的神经网络的原型。。**
>
> **版本号说明**: 当前版本号 (v5.1.3) 为先验机阶段版本号，遵循先验机的迭代规则。一旦正式定型为原型机，版本号将清零并作为独立项目重新发布。先验机与原型机分属不同项目阶段，不可混为一谈。
>
> 先验机与原型机的本质区别: 框架与基础设施的成熟度不同。如同胎儿与婴儿 —— 严格意义上均为生命体，但发展阶段完全不同。SGN 当前仍处于"胎儿期"，核心框架已搭建，但关键基础设施尚未完善。
>
> **当前已实现**: 整数化竞争学习、图模式记忆结构、多层竞争扩展（v5.1.3）
>
> **尚未完善**:
> - 颜色识别：计划已有，需等待 core 机制完善后统一接入
> - HC 时间库：制作中，将统一 level/整数化基础设施，接入时 core 需配合重写
> - 时间变量当前仅为计步器，未实现真正的时间维度处理
> - 上层神经元架构（组合逻辑、结构化信号输出）正在设计中，当前底层神经元（winner ID 输出）不足以支撑
> - 图模式与神经元的深度双向耦合尚未完成，当前为单向流水线
>
> **平台定位说明**: SGN 当前为 **PC 平台的验证框架代码**，**并非 MCU 部署版本**。未来发展方向包含 MCU 适配，但当前代码无法在嵌入式设备上直接运行。下载本代码获取的是算法验证平台，而非可部署的产品固件。
>
> **概念声明**: SGN 不是传统意义上的 SNN (脉冲神经网络) 或 DNN (深度神经网络)，拥有独立的概念体系和术语定义。部分概念可能与现有神经网络范式相似，但存在本质差异。当前优先实现的是记忆架构与竞争学习机制，而非神经元本体的完整功能。
>
> **使用建议**: 可将 SGN 视为"记忆先验框架"进行实验和扩展，但不应将其作为成熟的神经网络解决方案直接部署。v5.1.3 的多层扩展为底层多竞争层设计，不是层级抽象或概念判断的实现。
