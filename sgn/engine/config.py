...
def _register_defaults():
    ...
    defaults = [
        ...
        ConfigItem("SILENCE_SPECIALIZE_THRESHOLD", 10, int, (3, 50),
                   "静默专精触发阈值（融合自旧GATE_SPECIALIZE_THRESHOLD）", "学习参数", False),
        ...
        # 多层神经元
        ConfigItem("ENABLE_MULTI_LAYER_NEURON", True, bool, None,
                   "启用多层神经元架构", "高级选项", True),
        ...
        # v5.1.6 分批次训练
        # ============================================================
        ConfigItem("BATCH_TRAIN_ENABLED", True, bool, None,
                   "启用分批次训练", "学习参数", True),
        ConfigItem("BATCH_SIZE", 32, int, (2, 256),
                   "每批样本数", "学习参数", False),
        ConfigItem("BATCH_SHUFFLE", True, bool, None,
                   "每轮打乱样本顺序", "学习参数", False),
        ConfigItem("BATCH_SHUFFLE_SEED", 0, int, None,
                   "打乱随机种子(0=每轮不同)", "学习参数", False),

        # ============================================================
        # v5.1.6 L0 同级比较与合并
        # ============================================================
        ConfigItem("L0_PEER_COMPARE_ENABLED", True, bool, None,
                   "启用L0同级比较合并", "学习参数", False),
        ConfigItem("L0_MERGE_SIMILARITY", 0.75, float, (0.5, 0.99),
                   "L0合并相似度阈值", "学习参数", False),
        ConfigItem("L0_WEAK_SIGNAL_RATIO", 0.15, float, (0.01, 0.49),
                   "弱信号判定比例(低于此=噪点)", "学习参数", False),
        ConfigItem("L0_BINARIZE_THRESH", 50, int, (1, 99),
                   "L0二值化阈值", "学习参数", False),
        ConfigItem("L0_MERGE_BUFFER_THRESH", 2, int, (1, 10),
                   "补丁P4:合并累积缓冲阈值(连续N批才写回)", "学习参数", False),
        ...
        ConfigItem("L1_DECISION_LAYER", True, bool, None,
                   "启用L1决策层", "学习参数", True),
        ConfigItem("L1_FEATURE_LR", 0.3, float, (0.05, 0.8),
                   "L1特征模板赫布学习率", "学习参数", False),
        ConfigItem("ACTIVE_SET_DECAY", 0.1, float, (0.01, 0.5),
                   "激活集频次衰减率(通用,适用任意层)", "学习参数", False),
        ConfigItem("ACTIVE_SET_KEEP_THRESH", 0.3, float, (0.05, 1.0),
                   "激活集保留阈值(低于此淘汰)", "学习参数", False),
        ConfigItem("FEATURE_VARIANCE_THRESH", 100.0, float, (1.0, 10000.0),
                   "特征方差阈值(低于此降权,通用)", "学习参数", False),

        # ============================================================
        # v5.1.8 模板噪声膨胀修复
        # ============================================================
        ConfigItem("TEMPLATE_PRUNE_RATIO", 0.15, float, (0.0, 0.5),
                   "模板剪枝比例：OR合并后清除低频位的比例", "学习参数", False),
        ConfigItem("CONSOLIDATE_INTERVAL", 100, int, (10, 10000),
                   "模板定期收敛合并间隔(每N步)", "学习参数", False),
        ConfigItem("NORMALIZE_BY_TEMPLATE", True, bool, None,
                   "分类匹配度按模板有效位数归一化", "学习参数", False),
