...
class MatplotlibBackend(ChartBackend):
    ...
    def plot_neurons(self, core, path: Optional[str] = None) -> Optional[str]:
        ...
        # Bug #7 修复：多层模式合并所有层神经元（core.N 仅指向 L0）
        if getattr(core, 'multi_layer_enabled', False):
            all_neurons = []
            for pool in core.neuron_layers.values():
                all_neurons.extend(pool)
        else:
            all_neurons = core.N

        active = [_base_val(n) for n in all_neurons if not n["L"]]
        locked = [_base_val(n) for n in all_neurons if n["L"]]
    ...
    def plot_neurons(self, core, path: Optional[str] = None) -> Optional[str]:
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            ...
            if getattr(core, 'multi_layer_enabled', False):
                ...
                all_neurons = core.N
            for i, n in enumerate(all_neurons):
