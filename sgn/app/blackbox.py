...
def do_blackbox_verify(core, source=None):
    ...
    if source is not None and hasattr(source, 'patterns'):
        ...
        # Bug #2 修复：多层模式从 graphs 的标签提取可用标签
        available_labels = sorted(core.graphs.keys()) if hasattr(core, 'graphs') and core.graphs else []
        available_patterns = None
    elif core.templates:
    ...
    # Bug #2 修复：多层模式跳过模板数检查，改为检查 graphs
    is_multi = getattr(core, 'multi_layer_enabled', False)
    if not is_multi and not core.templates:
        print(f"\n  {C.YEL}⚠ 网络尚无模板，无法验证。{C.RST}")
        print(f"  {C.DIM}提示：黑箱训练可能未完成，或模板库为空。{C.RST}")
        return
    if is_multi and not getattr(core, 'graphs', {}):
        print(f"\n  {C.YEL}⚠ 网络尚无图，无法验证。{C.RST}")
        print(f"  {C.DIM}提示：黑箱训练可能未完成，或图为空。{C.RST}")
        return
    ...
    while True:
        ...
        if is_multi:
            # Bug #2 修复：多层模式用 classify_multi_layer 推理（不遍历空模板库）
            from engine.layers import classify_multi_layer
            pred, best_s = classify_multi_layer(core, intensity, d)
        else:
            best_s, pred = -1, '?'
        ...
        # Bug #2 修复：多层模式无 templates/_template_index，跳过模板参考显示
        if not is_multi:
            hit_templates = []
