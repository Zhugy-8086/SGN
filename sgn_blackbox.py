...
def do_blackbox_verify(core, source=None):
    """黑箱模式手动验证阶段"""
    from sgn_layers import extract_layers, combine_layers, match_bits
    from sgn_draw import draw_binary_grid, draw_index_grid, draw_intensity_grid
    from sgn_config import CONFIG, PATTERNS, LABELS
    from sgn_utils import C, box
    ...
    while True:
        ...
        print(f"  {C.DIM}└────────────────────────────────────────────────┘{C.RST}")
        draw_binary_grid(layers[0] if lc > 0 else 0, grid_size=grid_size)

        print(f"\n  匹配度: {C.val(best_s)}% [{match_bar}]  置信: {conf_str}")
        hit_templates = [(tlb, s) for tlb, tm, ts, thc in core.templates if (s := match_bits(tm, signature, d=d)) >= 50]
        if hit_templates:
