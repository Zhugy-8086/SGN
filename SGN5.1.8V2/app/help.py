...
def do_help():
  ...
  {C.BOLD}新增 CLI 参数{C.RST}
  python main.py --version          显示版本号
  python main.py --set KEY=VALUE     设置配置项(可多次指定)
  python main.py --preset quick      使用内置预设(quick/standard/precise)
  python main.py --status            输出网络状态后退出(非交互)
  python main.py --status --json     输出 JSON 格式状态
  {C.DIM}示例: python main.py --set MAX_NEURONS=512 --set SEED=123 --batch{C.RST}
""")
    hr(46)
    ...
    fixes = [
        ...
        ("切换矢量模式无效", "控制面板 [i] 输入源切换 → 选择矢量模式 → [Enter]返回\n"
                              "           必须返回控制面板再开始训练才会生效"),
