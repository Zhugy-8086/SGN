...
# --- 路径保护：确保无论从 run.py 启动还是直接运行 main.py，都能找到 engine/ 和 app/ ---
...
_import_errors = []
try:
    from engine.config import CONFIG, LABELS, D, DEFAULT_CONFIG
    from engine.config import save_config as cfg_save, load_config as cfg_load
except ImportError as _e:
    _import_errors.append(("sgn_config", _e))

try:
    from engine.utils import C, box, hr, gen_samples
except ImportError as _e:
...
try:
    from app.persist import save_model, load_model, autosave_check, check_resume
except ImportError as _e:
    _import_errors.append(("sgn_persist", _e))

try:
    from app.visual import do_stats, do_gauge, do_visualize, do_heatmap
except ImportError as _e:
    _import_errors.append(("sgn_visual", _e))

try:
    from app.interactive import control_panel, menu, homepage
except ImportError as _e:
...
try:
    from app.help import do_help
except ImportError as _e:
    _import_errors.append(("sgn_help", _e))

try:
    from app.cmd_registry import register_all_commands
except ImportError as _e:
...
try:
    from app.test import do_batch_test, do_confusion, do_noise_test
except ImportError as _e:
    _import_errors.append(("sgn_test", _e))

try:
    from app.report import do_plot
except ImportError as _e:
    _import_errors.append(("sgn_report", _e))

try:
    from engine.utils import compact_banner, compact_step, compact_summary
except ImportError as _e:
...
from app.test import do_batch_test, do_confusion, do_noise_test
from app.report import do_plot
from engine.utils import compact_banner, compact_step

from app.interactive import control_panel, menu, homepage
from app.training import run_training_loop, header, out_step
from app.help import do_help
from app.cmd_registry import register_all_commands
from engine.utils import gen_samples
from engine.hooks import HookRegistry
from engine.input import (
    create_default_source, create_file_source,
    DefaultCompositeNoise, PatternInputSource
...
v5.0 插件化架构: HookRegistry | ConfigRegistry | CommandRegistry |
    ...
    parser.add_argument("--custom-dataset", type=str, metavar="FILE",
                        help="自定义训练集文件路径（JSON格式，由函数工厂GUI生成）")
    parser.add_argument("--version", action="version", version="SGN-Lite v5.1.8")
    parser.add_argument("--set", type=str, action="append", metavar="KEY=VALUE",
                        help="设置配置项（可多次指定，如 --set MAX_NEURONS=512 --set SEED=123）")
    parser.add_argument("--preset", type=str, choices=["quick", "standard", "precise"],
                        help="使用内置预设: quick(快速验证)/standard(标准训练)/precise(精确训练)")
    parser.add_argument("--status", action="store_true",
                        help="输出当前网络状态后退出（非交互）")
    parser.add_argument("--json", action="store_true",
                        help="配合 --status 使用，输出 JSON 格式")
    return parser
...
def welcome():
    ...
    # 显示模块结构
    print(f"\n  {C.DIM}模块结构:{C.RST}")
    print(f"    {C.BOLD}核心层 (engine/):{C.RST}")
    print(f"      core.py           核心引擎 — 整数化竞争/校验/学习/模板合并 + Level 调度器")
    print(f"      config.py         配置注册表 / 动态UI")
    print(f"      hooks.py          事件总线 / 钩子系统")
    print(f"      input.py          输入管道 / 噪声模型 / 特征提取")
    print(f"      layers.py         层处理核心算法 (离散坐标/层级投影)")
    print(f"      level.py          层级调度器配置")
    print(f"      level_utils.py    层级调度器工具函数")
    print(f"      strategies.py     核心策略抽象 (学习率/削弱率/固化判定)")
    print(f"      utils.py          工具函数 / 颜色输出 / 日志")
    print(f"    {C.BOLD}图模式层 (graph/):{C.RST}")
    print(f"      graph.py          图数据结构定义")
    print(f"      stack.py          图构建与投影")
    print(f"      graph_match.py    图匹配与推理")
    print(f"      merge.py          跨图合并")
    print(f"    {C.BOLD}应用层 (app/):{C.RST}")
    print(f"      main.py           入口文件")
    print(f"      interactive.py    交互调度 (菜单/事件循环)")
    print(f"      commands.py       命令注册表")
    print(f"      cmd_registry.py   命令注册聚合")
    print(f"      panel.py          控制面板 (参数调整)")
    print(f"      training.py       训练循环")
    print(f"      test.py           批量测试 / 混淆矩阵")
    print(f"      visual.py         可视化 (训练过程网格显示)")
    print(f"      draw.py           基础绘图工具")
    print(f"      report.py         报告与图表导出")
    print(f"      metrics.py        评估指标")
    print(f"      backends.py       图表后端 (matplotlib/ASCII/CSV)")
    print(f"      storage.py        持久化策略抽象 (SQLite/JSON)")
    print(f"      persist.py        模型持久化 (加载/保存)")
    print(f"      blackbox.py       黑箱验证模式")
    print(f"      help.py           帮助手册")
    print(f"      noise_equivalent.py  噪声等效测试")
...
def _post_training_menu(core, samples, current_step, max_step, test_samples=None, source=None, is_paused=False):
    """训练结束后的交互菜单循环（公共逻辑）"""
    from app.interactive import menu
    while True:
...
def _maybe_rebuild_core(core, label=""):
    ...
    使用 ConfigRegistry._arch_modified_keys 精确追踪被修改的架构参数，
    而非比较当前值与默认值（避免"改了又改回"的误判）。

    Returns: 新的 core 对象（若重建）或原对象
    """
    from engine.config import is_config_modified, mark_config_synced, ConfigRegistry
    if not is_config_modified():
        return core

    # 获取被修改的架构参数（精确追踪，不依赖默认值比较）
    modified_keys = ConfigRegistry.get_arch_modified_keys()

    if modified_keys:
        seed = CONFIG.get("SEED", 42)
        log_print(f"\n  {C.info('ℹ')} 检测到架构参数变更 {sorted(modified_keys)}，正在重建网络...")
        log_print(f"  {C.DIM}   随机种子: {C.val(seed)} (神经元初始模板由种子决定){C.RST}")
        core = SGNCore(seed)
        ConfigRegistry.mark_arch_synced()
        mark_config_synced()
        # 显示重建后的网络状态
        if core.multi_layer_enabled:
            l0 = len(core.neuron_layers.get(0, []))
            l1 = len(core.neuron_layers.get(1, []))
            log_print(f"  {C.ok('✓')} 网络已重建: L0={l0} + L1={l1} = {l0+l1} 神经元 / 种子={C.val(seed)}")
        else:
...
def apply_cli_args(args):
    ...
    # --- 优先级 1: --preset 预设（最先应用，--set 可覆盖） ---
    if hasattr(args, 'preset') and args.preset:
        apply_preset(args.preset)

    # --- 优先级 2: --set KEY=VALUE 批量设置（最高优先级，可覆盖预设） ---
    if hasattr(args, 'set') and args.set:
        for item in args.set:
            if "=" not in item:
                log_print(f"  {C.err('x')} --set 格式错误: '{item}'（应为 KEY=VALUE）")
                continue
            ...
            if val_str.lower() in ("true", "false"):
                ...
                try:
                    ...
                    try:
                        ...
                        typed_val = val_str
            ok, msg = ConfigRegistry.set(key, typed_val)
            if not ok:
                log_print(f"  {C.err('x')} --set {key}={typed_val} 无效: {msg}")
            elif msg:
    ...
    # --custom-dataset 特殊处理：从函数工厂GUI生成的训练集文件导入
    if args.custom_dataset:
        ok, msg = ConfigRegistry.set("CUSTOM_DATASET_PATH", args.custom_dataset)
        if ok:
            ConfigRegistry.set("INPUT_SOURCE_TYPE", "custom")
        else:
            log_print(f"  {C.err('✗')} 命令行参数 CUSTOM_DATASET_PATH={args.custom_dataset} 无效: {msg}")
...
def _prepare_samples(args):
    ...
    关键行为：
      - pattern 模式使用 8×8 标准字符集（CharRegistry 插件，0-9 A-Z 共 36 字符）
    ...
    if source_type == "file":
        ...
        # 从函数工厂 GUI 生成的自定义训练集导入
        path = CONFIG.get("CUSTOM_DATASET_PATH", getattr(args, 'custom_dataset', None))
        if path:
            try:
                ...
                store = CustomDatasetStore(path)
                store.load()
                noise = DefaultCompositeNoise(CONFIG.get("FLIP_PROB", 0.15))
                source = CustomDatasetInputSource(store, noise_model=noise)
                log_print(f"  {C.ok('✓')} 自定义训练集已加载: {path}")
                log_print(f"  {C.info('ℹ')} 条目: {store.count()}  标签: {', '.join(store.get_labels())}")
            except Exception as e:
                print(f"  {C.err('X')} 自定义训练集加载失败: {e}")
                print(f"  {C.info('i')} 回退到内置字符模式")
                source = create_default_source()
        else:
            print(f"  {C.warn('⚠')} 未设置自定义训练集路径，回退到内置字符")
            source = create_default_source()
    else:
        # pattern 模式：使用 CharRegistry 默认 8×8 字符集（0-9 A-Z，共 36 字符），
    ...
    log_print(f"  {C.info('ℹ')} 训练噪声配置: {C.val(nt_label)} (p={noise_prob}) | 输入源: {C.val(source_type)} | 网格: {C.val(getattr(source, 'grid_size', 8))}")
...
def main():
    ...
    # 2.5. --set 最高优先级（在 --config 之后重新应用，确保 --set 不被覆盖）
    if hasattr(args, 'set') and args.set:
        from engine.config import ConfigRegistry
        for item in args.set:
            if "=" not in item:
            ...
            if val_str.lower() in ("true", "false"):
                ...
                try:
                    ...
                    try:
                        ...
                        typed_val = val_str
            ConfigRegistry.set(key, typed_val)
    ...
    if getattr(args, 'status', False):
        ...
        neurons = st.get('neurons', len(core.N)) if hasattr(core, 'N') else 0
        templates = st.get('templates', len(core.templates)) if hasattr(core, 'templates') else 0
        history_len = len(core.history) if hasattr(core, 'history') else 0
        mode = CONFIG.get("MODE", "full")
        if getattr(args, 'json', False):
            status_data = {
                "version": "SGN-Lite v5.1.8",
                "neurons": neurons,
                "templates": templates,
                "steps": history_len,
                "mode": mode,
            ...
            print(_json.dumps(status_data, indent=2, ensure_ascii=False, default=str))
        else:
            box("网络状态")
            print(f"  版本:    {C.val('SGN-Lite v5.1.8')}")
            print(f"  神经元:  {C.val(neurons)}")
            print(f"  模板:    {C.val(templates)}")
            print(f"  步数:    {C.val(history_len)}")
            print(f"  模式:    {C.val(mode)}")
            if getattr(core, 'multi_layer_enabled', False):
                l0 = st.get('layer0_count', '?')
                l1 = st.get('layer1_count', '?')
                print(f"  多层:    L0={C.val(l0)} / L1={C.val(l1)}")
            print(f"  输入源:  {C.val(CONFIG.get('INPUT_SOURCE_TYPE', 'pattern'))}")
            print(f"  噪声:    {C.val(CONFIG.get('NOISE_TYPE', 'composite'))} (p={C.val(CONFIG.get('FLIP_PROB', 0.1))})")
        close_log()
        return
    ...
    # 10. 交叉验证模式
    if args.cross_validate:
        from engine.utils import run_cross_validation
    ...
    while True:
        ...
        if args.auto is None:
            # 显示主页（模式选择 + 快速启动），接收返回值
            cp_result = homepage(core)
            if cp_result is None:
                # 【fix】用户按 q 退出主页，直接退出程序
                break
        ...
        if args.batch:
            ...
            # 交互模式
            try:
                current_step = run_training_loop(core, train_samples, max_step, delay_ms=0, test_samples=test_samples, source=source)
            except Exception as _e:
                import traceback, time
                ts = time.strftime("%Y%m%d_%H%M%S")
                crash_path = f"sgn_crash_{ts}.log"
            ...
            # 【fix】run_training_loop 返回整数步数，进入训练后菜单
