...
from engine.config import CONFIG, DiscreteCoordinate, SGNConstants
...
class DynamicGraph:
    ...
    def feature_similarity(
        ...
        try:
            ...
            ctx = scheduler.get_context(OperationType.COMPARE, source="graph_feature_similarity")
    ...
    def hebbian_merge(
        ...
        try:
            from engine.level import get_global_scheduler, OperationType
            scheduler = get_global_scheduler()
            ctx = scheduler.get_context(OperationType.ADD, source="graph_hebbian_merge")
            target_level = ctx.target_level
        except ImportError:
            # fallback: 使用最高层级
            all_levels = [f.level for f in source_node.feature_vector]
        ...
        if best_id is not None:
            ...
            if (source_node.label and target.label
                    and source_node.label != target.label):
                best_id = None
            else:
                # 更新状态
                ...
                # 特征加权平均（整数化，使用调度器决定的 level）
                min_len = min(len(target.feature_vector), len(source_node.feature_vector))
                for i in range(min_len):
                    t_idx = target.feature_vector[i].to_level(target_level).index
                    s_idx = source_node.feature_vector[i].to_level(target_level).index
                    new_idx = (t_idx * (100 - alpha) + s_idx * alpha) // 100
                    target.feature_vector[i] = DiscreteCoordinate(new_idx, target_level)
        ...
        # 添加为新节点（best_id is None 或标签守卫拒绝融合）
        new_node = GraphNode(
            node_id=-1,
