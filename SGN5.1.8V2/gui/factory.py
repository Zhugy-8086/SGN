...
class GUIFactory:
    """函数工厂主窗口 — 支持窗口大小自适应、8×8 网格（插件化字符集）、图片加载、训练集导出、多标签训练

    v5.1.5：移除 4×4 硬编码，默认网格改为 8×8，标签扩展为 0-9 A-Z 共 36 个字符。
    """

    def __init__(self, core=None, cli_args: Optional[Dict] = None):
        """初始化函数工厂主窗口

        Args:
            core: 可选，复用已有的 SGNCore 实例
            cli_args: 可选，命令行参数字典，用于控制启动配置。
                      支持的键: grid, char, label, angle, offset_x, offset_y,
                      scale, variants, export, variant_angle, variant_offset,
                      variant_scale, auto_train, train_steps, train_speed,
                      noise, noise_prob, dataset
        """
        # 命令行参数（空字典表示无参数启动）
        ...
        # 画布（默认 8×8，可通过 --grid 覆盖）
        self.canvas: Optional[DrawCanvas] = None
        self._recalc_layout()
        initial_grid = self.cli_args.get("grid", 8)
        if initial_grid not in GRID_SIZES:
            initial_grid = 8
        self.canvas = self._make_canvas(initial_grid)
        ...
        # 变换引擎（可通过 --angle/--offset-x/--offset-y/--scale 覆盖初始值）
        self.transform = TransformEngine()
        if "angle" in self.cli_args:
            self.transform.angle = self.cli_args["angle"]
        if "offset_x" in self.cli_args:
            self.transform.offset_x = self.cli_args["offset_x"]
        if "offset_y" in self.cli_args:
            self.transform.offset_y = self.cli_args["offset_y"]
        if "scale" in self.cli_args:
            self.transform.scale = self.cli_args["scale"]
        self.preview_intensity: Optional[List[int]] = None
        self.preview_grid_size = initial_grid  # 与初始画布网格一致

        # SGN 桥接
        self.bridge = SGNBridge(core=core)

        # 噪声配置（可通过 --noise/--noise-prob 覆盖）
        self._apply_cli_noise()
        ...
        # 命令行：加载指定数据集文件（--dataset FILE）
        if "dataset" in self.cli_args:
            ds_path = self.cli_args["dataset"]
            if os.path.exists(ds_path):
                if self.dataset_store.load(ds_path):
                    print(f"[函数工厂] 命令行加载数据集: {ds_path} ({self.dataset_store.count()} 条)")
                else:
                    print(f"[函数工厂] 警告: 数据集加载失败: {ds_path}")
            else:
                print(f"[函数工厂] 警告: 数据集文件不存在: {ds_path}")

        # 训练状态
        self.label = self.cli_args.get("label", "A")
        if len(self.label) > 1:
            self.label = self.label[:1]
        self.train_steps = self.cli_args.get("train_steps", 100)
        self.is_training = False
        self.train_target = 0
        self.train_speed = self.cli_args.get("train_speed", 5)
        ...
        # 命令行：加载字符到画布（--char C）
        if "char" in self.cli_args:
            self._load_char(self.cli_args["char"])

        # 命令行：生成变体（--variants N）
        if "variants" in self.cli_args:
            count = self.cli_args["variants"]
            self._generate_variant_dataset(count)

        # 命令行：导出变体（--export FILE）
        if "export" in self.cli_args:
            export_path = self.cli_args["export"]
            fmt = "csv" if export_path.lower().endswith(".csv") else "json"
            try:
                if not self.train_history:
                    self._generate_variant_dataset()
                export_training_dataset(self.train_history, export_path, self.canvas.grid_size, fmt)
                print(f"[函数工厂] 变体已导出: {export_path} ({len(self.train_history)} 条)")
            except Exception as e:
                print(f"[函数工厂] 导出失败: {e}")

        # 命令行：自动训练（--auto-train）
        if self.cli_args.get("auto_train"):
    ...
    def _apply_cli_noise(self):
        """根据命令行参数 --noise/--noise-prob 配置噪声模型

        支持的噪声类型: composite, gaussian, salt_pepper, block, none
        若未指定命令行参数则保持默认（bridge 自带 composite 噪声）。
        """
        noise_type = self.cli_args.get("noise")
        if noise_type is None:
            return  # 未指定，保持默认
        try:
            from engine.input import (
                DefaultCompositeNoise, GaussianNoise, SaltPepperNoise,
                BlockNoise, NoNoise,
            )
            prob = self.cli_args.get("noise_prob", 0.15)
            if noise_type == "composite":
                self.bridge.noise = DefaultCompositeNoise(prob)
            elif noise_type == "gaussian":
                self.bridge.noise = GaussianNoise(sigma=prob * 255)
            elif noise_type == "salt_pepper":
                self.bridge.noise = SaltPepperNoise(prob)
            elif noise_type == "block":
                self.bridge.noise = BlockNoise(prob, grid_size=self.canvas.grid_size)
            elif noise_type == "none":
                self.bridge.noise = None
            print(f"[函数工厂] 噪声已设置: {noise_type} (p={prob})")
        except Exception as e:
            print(f"[函数工厂] 噪声设置失败: {e}")
    ...
    def _build_ui(self):
        """构建右侧控制面板的所有 UI 组件（按钮、滑块、标签、输入框）

        组件按从上到下顺序排列：变换滑块 → 变换按钮 → 预览区 →
        标签按钮 → 训练控制 → 导出 → 模型管理 → 训练集管理 → 状态日志。
        每次窗口大小变化时重新调用以适应新尺寸。
        """
        ...
        # 标签按钮（四行 9 列，0-9 A-Z 共 36 个字符）
        self.label_buttons = []
        labels = list("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ")
        label_cols = 9
        btn_size = min(32, (rw - 20) // label_cols - 4)
        for i, lb in enumerate(labels):
            bx = rx + (i % label_cols) * (btn_size + 4)
            by = y + 22 + (i // label_cols) * (btn_size + 4)
            btn = Button(
        ...
        # 自定义标签输入框
        label_rows = (len(labels) + label_cols - 1) // label_cols
        y += label_rows * (btn_size + 4) + 10
    ...
    def _set_grid_size(self, gs: int):
        ...
        使用最近邻映射将旧网格的像素值搬到新网格：
        对于新网格中的每个像素 (x, y)，从旧网格取对应位置的值。
        """
        self._push_undo()
    ...
    def _update_transform(self):
        """从滑块读取变换参数，应用到画布数据生成预览 intensity"""
    ...
    def _generate_variant_dataset(self, count: int = 100):
        """生成指定数量的变体并保存到内存（用于导出或训练）
        ...
        # 从命令行参数读取变体随机范围（若指定了 --variant-angle 等）
        v_angle = self.cli_args.get("variant_angle", 15.0)
        v_offset = self.cli_args.get("variant_offset", 6)
        v_scale = self.cli_args.get("variant_scale", 0.2)

        self.train_history = []
        for _ in range(count):
            # 使用指定范围的随机变换参数
            self.transform.angle = random.uniform(-v_angle, v_angle)
            self.transform.offset_x = random.randint(-v_offset, v_offset)
            self.transform.offset_y = random.randint(-v_offset, v_offset)
            self.transform.scale = 1.0 + random.uniform(-v_scale, v_scale)
        ...
        self._update_transform()
        self._update_status(f"已生成 {count} 个变体\n标签: {self.label}  网格: {gs}×{gs}\n点击导出CSV/JSON保存")
    ...
    def _test_current(self):
        """对当前预览 intensity 执行分类测试，显示预测标签和匹配度"""
    ...
    def _do_training_frame(self):
        ...
        根据 train_speed 控制每帧训练步数，达到目标步数后自动停止。
        多标签模式下从训练集循环采样，单标签模式下从当前画布生成变体。
        """
    ...
    def _draw_preview(self):
        """绘制变换预览区：将 preview_intensity 渲染为灰度像素方块"""
        if self.preview_intensity is None:
            return
        gs = self.canvas.grid_size
    ...
    def _draw_info_panel(self):
        """在画布下方绘制信息面板（网格大小、变换参数、标签、网络状态、上次测试结果）"""
    ...
    def run(self):
        """主事件循环：处理输入事件、执行训练帧、渲染所有 UI 组件

        快捷键：空格=训练/暂停  T=测试  R=随机变换  C=清空
                E=橡皮擦  Ctrl+Z=撤销  Ctrl+Y=重做  ESC=退出
        """
        self._update_transform()
        running = True
        while running:
