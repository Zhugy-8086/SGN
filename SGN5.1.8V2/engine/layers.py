...
def match_bits(a: int, b: int, d: int = None,
               normalize_by_template: bool = False) -> int:
    """匹配度 0-100（自动适应窗口大小）

    Args:
        a, b: 待比较的掩码整数
        d: 窗口像素总数（默认从 CONFIG['D'] 读取，运行时推导）
        normalize_by_template: 若为 True，分母使用模板有效位数（a 中非零位数）
            而非固定 D，消除膨胀模板因分母固定而获得虚高匹配的问题
    """
    if d is None:
        from engine.config import CONFIG
        d = CONFIG.get("D", 64)
    mask = (1 << d) - 1
    if normalize_by_template:
        # 按模板有效位数归一化：匹配度 = 重叠位(同时为1) / 模板有效位
        denom = popcount(a & mask)
        if denom == 0:
            return 0
        overlap = popcount((a & b) & mask)
        return (overlap * 100) // denom
...
def classify_sample(core, intensity: List[int], d: int = None) -> Tuple[str, int]:
    ...
    best_s, pred_lb = -1, '?'
    from engine.config import CONFIG
    norm = CONFIG.get("NORMALIZE_BY_TEMPLATE", True)
    for tlb, tm, ts, thc in core.templates:
        s = match_bits(tm, signature, d=d, normalize_by_template=norm)
        if s > best_s:
...
def classify_multi_layer(core, intensity: List[int], d: int = None) -> Tuple[str, int]:
    """多层神经元模式推理（v5.1.9: 72 维图特征 + Top-2 跨类平均分类）

    数据流与 multi_layer_l1_phase 一致，但不做赫布学习/不更新图。
    """
    if d is None:
        from engine.config import CONFIG
    ...
    # 2. Layer 0 神经元竞争（v5.1.8: 统一与训练一致的静默竞争）
    layer0_pool = core.neuron_layers.get(0, [])
    from engine.multi_layer_train import l0_compete
    winners_l0_raw = l0_compete(core, intensity, layers, layer_count)
    if not winners_l0_raw:
        return '?', 0
    winners_l0 = [
        (w["speed"], w["match"], w["nid"])
        for w in winners_l0_raw
    ]
    ...
    # v5.1.9: 按类别 Top-2 平均分跨类比较（替代全局 argmax，防止单类坍缩）
    label_scores = {}
