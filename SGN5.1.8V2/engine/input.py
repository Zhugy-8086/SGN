...
class BlockNoise(NoiseModel):
    ...
    def __init__(self, block_size: int = 2, prob: float = 0.2, grid_size: Optional[int] = None):
...
class VectorPatternSource(InputSource):
    ...
    def __init__(self, formula_type: str = "line", grid_size: int = 8,
        ...
        # [v5.1 compatibility layer] catear renamed to arch, old config/cli auto-mapped
        if self.formula_type == "catear":
            self.formula_type = "arch"
        self.grid_size = grid_size
    ...
    def _contrast_stretch(self, intensity: List[int]) -> List[int]:
        """Percentile clipping stretch, defends against salt-and-pepper noise amplification (2%~98% percentile)"""
        ...
        if high <= low:
            return intensity[:]
        return [min(255, (v - low) * 255 // (high - low)) for v in intensity]
    ...
    def _render(self, label: str) -> List[int]:
        ...
        if self.formula_type == "line":
            ...
            raw = self._render_catear(params["ear_spacing"], params["ear_radius"],
                                      params["open_angle"],
                                      orientation=params.get("orientation", "up"),
                                      variant=params.get("variant", "filled"))
        else:
            raise ValueError(f"Unknown vector formula type: {self.formula_type}")
...
def create_mixed_vector_source(
    ...
    """创建多公式混合矢量输入源（多类识别训练）"""
    if formulas is None:
        formulas = ["line", "circle", "sine", "arch", "leaf"]
    noise = DefaultCompositeNoise(noise_prob)
...
class _MixedVectorSource(InputSource):
    ...
    def __init__(self, formulas, grid_size, noise, samples_per_formula, seed):
        self.grid_size = grid_size          # [v5.1] Save grid_size for external access
        self.sources = []
...
def create_default_source(noise_prob: float = None) -> InputSource:
    """创建默认输入源（内置 8×8 标准字符集 + 复合噪声）

    v5.1.5：字符模板来自 CharRegistry 默认插件（0-9, A-Z，共 36 字符，8×8），
    替代旧版 4×4 硬编码 PATTERNS。SGNCore 首次 train 时自动推导 D=64。

    Args:
        noise_prob: 噪声翻转概率，若为 None 则从 CONFIG['FLIP_PROB'] 读取

    Returns:
        PatternInputSource 实例（8×8 字符集，36 标签）
    """
    from engine.config import PATTERNS, CONFIG
...
class StandardCharSource(InputSource):
    ...
    使用 CharRegistry 默认字符集（0-9, A-Z，共 36 字符），支持噪声注入。
    v5.1.5：从直接导入 STANDARD_CHARS_8x8 改为通过 CharRegistry 插件系统获取，
    支持运行时切换字符集插件。
    """

    def __init__(self, chars: str = None, noise_model: NoiseModel = None, grid_size: int = 8):
        from engine.config import CharRegistry
        # 从插件注册表获取字符模板和网格大小（默认 "standard_8x8"，8×8）
        self.char_lib = CharRegistry.get_patterns()
        self.grid_size = CharRegistry.get_grid_size()
        if chars is None:
        ...
        self.d = self.grid_size * self.grid_size
