# SGN-Lite v5.1.7 架构设计文档

> **版本说明**：本文档从 v5.1.3 更新至 v5.1.7，涵盖 v5.1.5 Level 调度器、v5.1.6 自适应静默机制/Bug修复/L0 同级比较/L1 决策层、v5.1.7 模块化引擎拆分全部架构变更。
>
> **旧文档**：`ARCHITECTUREv5.1.3.md`（已归档），文件名中的 `sgn_` 前缀在 v5.1.7 中全部改为无前缀命名（`sgn_core.py` → `engine/core.py`）。
...
| `engine/l1_decision.py` | ~455 | L1 桶化/链条化/输出模式/聚类/回馈/P2纠错/匹配 |
...
### 3.7 L1 决策层与自组织分类 (v5.1.6)

**核心思想**：L1 神经元不输出标签，输出"判断模式向量"（`L1_OUTPUT_PATTERN_DIM` 维）。输出相似的 L1 神经元自然聚成一类，聚类标签 = 涌现分类。
...
```python
def l1_generate_output_pattern(core, nid, l0_winners, graph_features):
    """生成 L1 判断模式向量（不是标签，是判断倾向）"""
    # 1. 基于 T_features 对图特征的响应
    # 2. 基于 T_l0_active 对 L0 激活的响应
    # 3. 组合为 L1_OUTPUT_PATTERN_DIM 维向量
    # 4. 存入 neuron["output_pattern"]
```

#### 3.7.3 L1 同级比较与聚类

- `l1_peer_compare_and_cluster(core)`：比较所有 L1 神经元的 `output_pattern`
- 相似度超过 `L1_CLUSTER_SIMILARITY`（默认 0.70）归为一类
- 聚类标签 = 涌现分类
...
判断被验证为正确时，L1 回馈调整图模式（中间层）：

```python
def l1_feedback_to_graph(core, nid, intensity):
    """L1 回馈：正确判断时调整图模式"""
    # 强度由 L1_FEEDBACK_STRENGTH（默认 0.1）控制
```
...
1. **簇置信度**：每个簇根据判断正确/总次数计算置信度
2. **周期性重组**：每 `L1_CLUSTER_REORG_INTERVAL` 步重组一次
3. **负反馈拆簇**：置信度低于 `L1_CLUSTER_CONFIDENCE_THRESH`（默认 0.5）时拆簇
...
def train_batch(core, samples, max_step):
    ...
    while step < max_step:
        ...
        # 批次结束后触发 L0 同级比较 + L1 聚类
        if L0_PEER_COMPARE_ENABLED:
            l0_peer_compare_and_merge(core)
        if L1_PEER_COMPARE_ENABLED:
            l1_peer_compare_and_cluster(core)
```
...
| `engine/l1_decision.py` | 12 个函数 | `bucket_winners_by_label()`, `chain_winners()`, `l1_generate_output_pattern()`, `l1_peer_compare_and_cluster()`, `l1_feedback_to_graph()`, `match_layer1()`, `compute_l1_predicted_label()` (v5.1.7-patch) |
...
**问题**：原 `l1_verified` 依赖 `specialization`/`cluster_id`，而这两者又依赖 `verified=True` 才能设置——冷启动死锁，L1 永远无法被验证。

**解法**：新增 `compute_l1_predicted_label()`（[`engine/l1_decision.py`](../engine/l1_decision.py)），将 L1 神经元的 `T_features` 与所有已积累的图匹配，取最佳匹配图的标签作为预测标签。完全不依赖 `specialization`/`cluster_id`，打破死锁。
...
**问题**：[`engine/layers.py`](../engine/layers.py) 的 `classify_multi_layer` 有三条重叠分类路径——A（`cluster_id` 涌现）、B（`specialization` 兜底）、C（遍历标签匹配）。三路径与训练侧 `_compute_l1_predicted_label` 并行演化，导致训练/推理不一致。

**解法**：删除路径 A/B 约 60 行，统一为复用 `compute_l1_predicted_label`：
...
> **未处理遗留**：`T[0] % 10000` 模丢失（特征提取层问题，未绑定实现，按方案文档延后处理）。
...
[多层模式]                       [L1 决策层模式]
classify_multi_layer()          涌现分类（v5.1.7）
  → L0 竞争 → 图特征              → L1 输出判断模式
  → L1 匹配 → 遍历 graphs         → 查 cluster_id → 涌现标签
  → 取最高分标签                  → backward compatible fallback
```

**涌现分类向后兼容**：`L1_DECISION_LAYER=False` 或无 `cluster_id` 时，自动回退到 `classify_multi_layer` 遍历标签匹配。
...
| L1_DECISION_LAYER | True | L1 决策层总开关 |
| L1_OUTPUT_PATTERN_DIM | 64 | 输出判断模式维度 |
| L1_PEER_COMPARE_ENABLED | True | L1 同级比较聚类 |
| L1_CLUSTER_SIMILARITY | 0.70 | 聚类相似度阈值 |
| L1_FEEDBACK_TO_GRAPH | True | L1 回馈图模式 |
| L1_FEEDBACK_STRENGTH | 0.1 | 回馈强度 |
| L1_CLUSTER_CONFIDENCE_THRESH | 0.5 | 簇置信度阈值 (P2) |
| L1_CLUSTER_REORG_INTERVAL | 10 | 周期性重组间隔 (P2) |
...
> **版本公开说明**：v5.1.6 为内部开发版本。v5.1.7 是首个公开版本，在 v5.1.6 全部功能基础上完成技术债清理和模块化重构。
