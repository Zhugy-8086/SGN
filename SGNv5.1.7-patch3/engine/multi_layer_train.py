...
def multi_layer_l1_phase(
    ...
    for bucket_label, bucket_winners in buckets.items():
        ...
        graph_features, l0_active_list = core._extract_graph_features(
            ...
            [w["nid"] for w in bucket_winners]
        )

        # 记录主桶特征（用于 L1 竞争）
        if bucket_label == label or not primary_graph_features:
    ...
    for i in active_l1:
        ...
        # v5.1.7: 生成输出判断模式
        output_pattern = core._l1_generate_output_pattern(
            n, primary_graph_features, primary_l0_active_list
        )
        n["output_pattern"] = output_pattern
        matches_l1.append({
            "nid": i,
            "match": match_l1,
            "speed": speed_l1,
    ...
    # 8. 最终分类验证（v5.1.7-patch: 基于图匹配的 L1 预测标签，打破冷启动死锁）
    # 不依赖 specialization/cluster_id（它们依赖 verified=True 才能被设置，会导致死锁），
    # 而是将 L1 神经元的 T_features 与所有图匹配，最佳匹配图标签作为预测标签。
    l0_active_nids = [w["nid"] for w in winners_l0]
    l0_verified = core._verify(intensity, layers, layer_count)

    # 预计算所有图的图特征（本步内不变，供所有 L1 winner 复用，避免重复提取）
    ...
    # 10. v5.1.7: L1 回馈图模式 + P2 负反馈拆簇
    if CONFIG.get("L1_DECISION_LAYER", True) and winners_l1:
        best_l1_n = layer1_pool[winners_l1[0]["nid"]]
        core._negative_feedback_uncluster(best_l1_n, l1_verified)
        primary_graph = core.graphs.get(primary_bucket_label)
        if primary_graph:
            core._l1_feedback_to_graph(
                best_l1_n, primary_graph, primary_graph_features, l1_verified
            )

    # 11. enc_r 鼓励衰减（全局 O(N)）
    ...
    # 12. 返回 info + _l1_outputs（供批次层收集做 L1 同级比较）
    # v5.1.7-patch: info 显示 L1 分类校验结果（用户可见的 ✓/✗ 反映真实分类）
    info = make_multi_layer_info(
        core, label, l1_verified, matches_l1[0]["match"],
        total_proj_nodes, len(winners_l1)
    )
    # v5.1.7-patch: 每个 L1 winner 独立计算 verified（复用图特征缓存）
    # v5.1.7-patch2: compute_l1_predicted_label 返回 (label, score)
    l1_outputs = []
    for w in winners_l1:
        n = layer1_pool[w["nid"]]
        pred, _ = core._compute_l1_predicted_label(
        ...
        if pred is None:
            ...
            v = (pred == label)
        l1_outputs.append((w["nid"], w["output_pattern"], v, label))
    info["_l1_outputs"] = l1_outputs
...
def train_batch_multi_layer(core, batch: List) -> List[Dict]:
    ...
    batch_info = []
    all_l1_outputs = []  # v5.1.7: 收集批次内所有 L1 输出供同级比较
    ...
    # v5.1.7 3.4: L1 同级比较与自组织聚类 + P2 周期性重组
    if CONFIG.get("L1_PEER_COMPARE_ENABLED", True) and all_l1_outputs:
        layer1_pool = core.neuron_layers[1]
        core._l1_peer_compare_and_cluster(all_l1_outputs, layer1_pool)
        # 补丁 P2: 周期性重组（每 N 批次）
        core._batch_counter = getattr(core, '_batch_counter', 0) + 1
        if core._batch_counter % CONFIG.get("L1_CLUSTER_REORG_INTERVAL", 10) == 0:
            core._cluster_reorganization(layer1_pool)

    # 清理内部字段（不输出给用户）
    for info in batch_info:
        info.pop("_l1_outputs", None)
