...
def do_visualize(core):
    ...
    # Bug #6 修复：多层模式可视化 graphs 结构（templates 为空）
    if multi_layer:
        if not getattr(core, 'graphs', {}):
            print(f"  {C.warn('⚠')} 无图数据可显示（请先训练）")
            return
        print(f"\n  {C.BOLD}多层结构可视化{C.RST} {C.CYN}[多层模式]{C.RST}")
        l0_pool = core.neuron_layers.get(0, [])
        l1_pool = core.neuron_layers.get(1, [])
        l0_active = sum(1 for n in l0_pool if not n["L"])
        l1_active = sum(1 for n in l1_pool if not n["L"])
        print(f"  Layer0: {l0_active}/{len(l0_pool)} 活跃 | Layer1: {l1_active}/{len(l1_pool)} 活跃")
        for label, g in sorted(core.graphs.items()):
            print(f"\n  {C.BOLD}标签 '{label}'{C.RST}: {g.get_total_nodes()}节点, 最高层 L{g.get_max_layer()}")
            layer_dist = {}
            for n in g.nodes.values():
...
def do_heatmap(core):
    ...
    if getattr(core, 'graph_mode', False) or getattr(core, 'multi_layer_enabled', False):
        mode_tag = "[图模式]" if getattr(core, 'graph_mode', False) else "[多层模式]"
        print(f"\n  {C.BOLD}热力图{C.RST} {C.CYN}{mode_tag}{C.RST}")
        print(f"  {C.YEL}⚠ 当前模式下热力图暂不支持（模板为空）{C.RST}")
...
def do_stats(core):
    ...
    if getattr(core, 'graph_mode', False):
        ...
        total_nodes = sum(g.get_total_nodes() for g in core.graphs.values()) if hasattr(core, 'graphs') else 0
        max_layer = max((g.get_max_layer() for g in core.graphs.values()), default=0) if hasattr(core, 'graphs') and core.graphs else 0
        print(f"  {C.CYN}[多层模式]{C.RST}")
        print(f"  神经元: {C.val(st['neurons'])} (L0={st.get('layer0_count','?')}/L1={st.get('layer1_count','?')}) 活跃:{C.GRN}{st['active']}{C.RST}")
        print(f"  图: {C.val(st['templates'])} 个标签 | 节点: {C.val(total_nodes)} 最高层: L{max_layer}")
        if hasattr(core, 'graphs') and core.graphs:
            for label, g in sorted(core.graphs.items()):
                layer_dist = {}
                for n in g.nodes.values():
...
def do_gauge(core):
    ...
    graph_mode = getattr(core, 'graph_mode', False)
    multi_layer = getattr(core, 'multi_layer_enabled', False)
    use_graphs = graph_mode or multi_layer
    ...
    if graph_mode:
        ...
        total_nodes = sum(g.get_total_nodes() for g in core.graphs.values()) if hasattr(core, 'graphs') else 0
        print(f"  {C.CYN}[多层模式]{C.RST}")
        print(f"  神经元: {C.GRN}{st['active']}{C.RST} 活跃 | {st['neurons']} 总数 (L0={st.get('layer0_count','?')}/L1={st.get('layer1_count','?')})")
        print(f"  图: {C.CYN}{st['templates']}{C.RST} 个标签 | 节点: {C.val(total_nodes)}")
    else:
