...
def _classify(core, intensity, d=None):
    ...
    v5.1.7-patch2: 委托到 classify_utils，返回 2 元组（向后兼容）
    """
    from app.classify_utils import classify_with_mode
    return classify_with_mode(core, intensity, d)[:2]  # 截掉 mode


def do_inference(core, source=None):
    """交互推理 - 支持连续测试，按 q 退出"""
    from engine.config import PATTERNS, LABELS, CONFIG
    ...
    while True:
        ...
        for li in range(lc):
            ...
            draw_binary_grid(layers[li], grid_size=int(d**0.5))
        # 显示合并签名
        combined = combine_layers(layers)
        combined_bits = bin(combined).count('1')
        ...
        draw_binary_grid(combined, grid_size=int(d**0.5))
        print(f"\n  {C.BOLD}{'─'*44}{C.RST}")
...
def do_batch_test(core, test_samples=None):
    ...
    correct = 0
    total = 0
    from app.classify_utils import classify_pass
    for intensity, label in test_samples:
        passed, _, _, _ = classify_pass(core, intensity, label)
        if passed:
            correct += 1
        total += 1
...
def do_confusion(core, test_samples=None):
    ...
    from engine.utils import mini_bar, C
...
def do_noise_test(core, test_samples=None, noise_model=None, source=None):
    ...
    from engine.config import CONFIG, LABELS, PATTERNS
    from engine.utils import hr, C
    ...
    if noise_model is None:
        ...
        grid_size = int(getattr(core, 'grid_size', CONFIG.get("D", 64) ** 0.5))
        ...
        if choice == "1":
            ...
            from engine.input import GaussianNoise
            ...
            from engine.input import SaltPepperNoise
            noise_cls = lambda p: SaltPepperNoise(noise_prob=p)
            param_values = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]
            fmt = lambda v: f"{v:>5.2f}"
        elif choice == "4":
            from engine.input import BlockNoise
    ...
    if source is not None and hasattr(source, 'patterns'):
        ...
        if source.formula_type == "mixed":
            ...
            test_labels = {"line": ["LINE"], "circle": ["CIRCLE"], "sine": ["SINE"], "arch": ["ARCH"], "leaf": ["LEAF"]}.get(source.formula_type, ["UNKNOWN"])
        source_type = "vector"
    elif source is not None and hasattr(source, 'sources'):
        test_labels = []
        for src in source.sources:
                    test_labels.extend({"line": ["LINE"], "circle": ["CIRCLE"], "sine": ["SINE"], "arch": ["ARCH"], "leaf": ["LEAF"]}.get(src.formula_type, ["UNKNOWN"]))
    ...
    for param_val in param_values:
        ...
        if source_type == "pattern":
            ...
            from engine.input import VectorPatternSource
