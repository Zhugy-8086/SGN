...
def _advanced_menu(core):
    ...
    while True:
        ...
        print(f"  {C.CYN}[7]{C.RST} 矢量猫耳 (8×8){C.GRN} ★{C.RST}" if src_type == "vector" and formula == "catear" else f"  {C.CYN}[7]{C.RST} 矢量猫耳 (8×8)")
        ...
        if choice == "q" or choice == "":
            ...
            ConfigRegistry.set("INPUT_SOURCE_TYPE", "vector")
            ConfigRegistry.set("VECTOR_FORMULA", "catear")
            print(f"  {C.ok('✓')} 已切换到矢量猫耳 (网格: {grid})")
        elif choice == "6":
            ...
            print(f"  {C.CYN}[1]{C.RST} line  {C.CYN}[2]{C.RST} circle  {C.CYN}[3]{C.RST} sine  {C.CYN}[4]{C.RST} catear  {C.CYN}[5]{C.RST} mixed")
            try:
                fc = input("  选择: ").strip()
                fm = {"1": "line", "2": "circle", "3": "sine", "4": "catear", "5": "mixed"}
                if fc in fm:
