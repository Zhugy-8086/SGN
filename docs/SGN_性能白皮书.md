...
> **解读**：B=16 继续改善（差距 9.2x→6.0x），B=4 略回退、B=8 持平；但 PyTorch 侧自身也明显变慢（B=16: 13.41 vs 9.43ms），说明本次环境负载更大、绝对数值不可直接横比。综合判定为测量噪声与环境差异，**未见一致、明确的代码层优化收益**。MSint dot_split SIMD 方案见 [dot_split_simd_optimization_plan_2026_08_13.md]。
...
### 3.7 公平对比基准（2026-09-07 重构：三方同任务口径）

> **对比原则（用户拍板重构）**：旧口径"SGN fp32 vs PyTorch"不公平——以
> SGN 最弱的 float 训练速度对标工业级 BLAS 框架，既无法体现 SGN 的优势
> （整数/量化路径），也容易被误读为"SGN 主张超越 PyTorch 效率"。重构后
> 的口径：**只比"大家都有"的功能**（同任务 MLP 784-256-128-10 + ReLU +
> CE、同 batch、同精度 f32、同硬件、fwd+bwd 墙钟中位 30 轮），三方同框。

实测（Core Ultra 5 225，单进程，`KMP_DUPLICATE_LIB_OK=TRUE`；脚本
`engine/sgn/tests/bench_fair_compare.py`，2026-09-07）：

| 实现 | ms/iter（中位） | vs numpy |
|---|---|---|
...
**诚实读数**：
1. SGN float 训练不主张超越 PyTorch/MKL——工业 BLAS 框架在该维度更快，
   这是架构差异（自定义内核 vs 深度优化的 vendor BLAS），白皮书如实呈现；
2. SGN 相对 numpy 手写基线 2.04×——框架价值在于提供 autograd/层抽象/
   checkpoint/实验框架等基础设施（numpy 手写无这些），同时保持同量级速度；
3. **SGN 的差异化优势在 float 训练之外的维度**：整数/量化路径（§5 的
   SIMD 点积 8.9–95× 为整数域对标量）、嵌套量化一次量化多档取用、
   Level 调度、LeveledState 平面推理（状态带宽 4.00–4.57×，见 §9）——
   这些是 numpy/PyTorch 不提供的能力。
...
| AES-CTR PRNG | 6 项功能测试全部通过 | 确定性、SR 非零率 60.1%（口径=SR 输出中非零元素占比，**非**"向上取整概率"；原始测试输入已失考，见 [SR算法审查与dot8溢出_2026_09_05.md] 附录 A） |
...
- 决策层/控制层/128 位融合不做；`dot_split_leveled` 的标量分组循环已通过「去逐元素堆分配 + 消除 std::map/cursor 热循环查找」两轮优化完成（分组阶段 -85%~-90% gap，见 [dot_split_leveled_grouping_heap_overhead_2026_08_14.md]）。

**MSint 拆分点积新增接口（2026-08-14，窄路径 SIMD 之上）**：

- **低位对角裁剪（`trim_high_diag` 参数 + `dot_fused_i32` 消费端）**：4 位档（n=8）裁剪高位对角 m≥8 的点积，省 **43.75% ALU**，int32 截断结果 bit-exact（`validate_low_diag_trim_int32.py` 69/69、`validate_low_diag_trim_kernel.py` 189/189 全 PASS）。整调用口径下裁剪收益被拆分/打包瓶颈稀释（无系统性收益），但在摊销接口 `narrow_dot_prepared4` 上兑现（**-44.65%**，K=65536）。`trim_high_diag` 贯穿 `narrow_group_dot` / `narrow_dot` / `narrow_dot_prepared4` / `dot_split`；`dot_fused_i32(w, x, split_bits=4)` 为消费端直接调用入口（内部走裁剪路径）。详见 [dot_split_low_diag_trim_2026_08_14.md]。
...
方案细节、偏置修正数学推导与实测数据见 [dot_split_simd_optimization_plan_2026_08_13.md]。
...
与内存池（P1，§7）同属 [engine_infrastructure_plan_2026_08_14.md] 的"基础设施三件套"，两者均已落地并验证：
...
## 9. 嵌套量化与层 2 平面推理（2026-09-07 追加）

> 方向状态：存活机制 = 开环训练配方（TF 温度前置 + WF 静态水填充）；
> 本节记录其推理期兑现路径（层 2 LeveledState）的实测读数。数据源：
> run_l2_w9_sysacct_first.py / run_l2_w9_wf25_mixed.py / W5–W7 测试。

### 9.1 一次量化全谱取用（精度-成本谱）

嵌套量化：SR 量化到最细格 u 产出单一 int32 码字，各档视图 = 码字截位
（切换零重采样噪声）。档位误差-成本谱（n=2000 全 int32 域码字实测，
S3 衔接评估 B3）：

| 档 | 视图码域 | 计算路径 | 点积相对误差 | 成本 |
|---|---|---|---|---|
| Q4 | ±8（floor） | unpack + dot8 | 1.08e-1 | 1 pass |
| Q8 | ±128（floor） | dot8 单 pass | 4.71e-3 | 1 pass |
| Q16 | int16（pair） | dot8×2 | 3.03e-5 | 2 pass |
| Q32 | int32（4 平面） | split-dot | 0（精确） | 4 pass |

### 9.2 状态带宽账（LTC N=64/T=28/10000 样本，解析账 = 埋点精确复算）

| 配置 | f32 状态搬运 | 平面状态搬运 | 倍率 |
|---|---|---|---|
| 全 Q8 | 143,360,000 B | 35,840,000 B | **4.00×** |
| WF-25 混合（25% Q4） | 143,360,000 B | 31,360,000 B | **4.571×** |

两读数与布局公式 4/(bytes-per-elem) 精确一致；质量侧：全 Q8 平面路径
对 f32 推理 Δ−0.05 点（V-L2W8 门 ≤0.5），WF-25 混合表零质量成本
（V-L2WF G1，3/3 seeds WF25≥Q8）。

### 9.3 精度守卫（W8，3 seeds × 120ep 训练后回放）

| 模式 | Δacc（均值） | 门 |
|---|---|---|
| Q8 全档（int8-w 部署形态） | −0.05 点 | ≤0.5 ✓ |
| TF 日程原样回放 | −0.05 点 | ≤2 ✓ |
| RTN 重建变体（report-only） | −0.01 点 | — |

延迟口径：以上为精度账；延迟守卫（S3 ≤1.10×）的正式判定待层 3 native
批量绑定——numpy 参考实现 5.5× 为指示值非结论。
