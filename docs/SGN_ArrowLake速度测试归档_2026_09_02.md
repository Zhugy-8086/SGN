...
## 8. 跨平台兼容对照（2026-09-03 补齐）

> 本归档（本机）与两份 EPYC 归档（Zen 4 / Zen 5）共同构成**一套代码、三平台各自
> 启用最优后端且全部 bit-exact**的兼容性证据。兼容性是设计出来的（per-file ISA +
> 运行时 CPUID），也是测出来的（§2 的 AVX-VNNI CPuID 双重错位 bug 正是"单平台验证
> 遮蔽平台相关缺陷"的实证——需多平台组合才能暴露并闭环）。

| 平台 | 微架构 | 指令集 | 运行时后端 | 正确性 | dot8 峰值 Mops/s |
|---|---|---|---|---|---|
| Intel Arrow Lake（本机，Windows） | 6P+4E | AVX2, AVX-VNNI；无 AVX-512 | `avxvnni`（256 位） | simd 238/238 | 93,516（K=4096） |
| AMD EPYC 9Y24（Zen 4，Ubuntu） | Zen 4 | AVX2, AVX-512VNNI；无 256 位 VNNI | `avx512vnni`（512 位） | simd 238/238 + gemm 4819/4819 | 56,228（K=65536，2vCPU） |
| AMD EPYC 9K65（Zen 5，Ubuntu） | Zen 5 | AVX2, AVX-512VNNI | `avx512vnni`（512 位） | simd 238/238 | 89,821（K=65536） |

> ⚠️ 吞吐差异反映指令宽度 / 频率 / 虚拟机资源（桌面 vs 2-3 vCPU VM），**不构成跨平台
> 性能结论**——同一代码在各自平台启用最优后端、数量级正确，即为兼容性的完成态。
> 详见 [Zen 4 归档](SGN_EPYC速度测试归档_2026_08_31.md) §0 与
> [Zen 5 归档](SGN_EPYC_Zen5速度测试归档_2026_09_03.md) §0。
