...
2. [环境要求](#2-环境要求)
3. [构建与安装](#3-构建与安装)
   - [3.5 常见构建问题](#35-常见构建问题)
4. [五分钟快速上手](#4-五分钟快速上手)
5. [双模式系统](#5-双模式系统)
   - [5.2 稳定模式（sgn.nn 子模块）](#52-稳定模式sgnnn-子模块)
   - [5.3 调试模式（自由函数）](#53-调试模式自由函数)
   - [5.4 混合模式](#54-混合模式)
6. [核心 API 速查](#6-核心-api-速查)
7. [sgn.level 子模块（Level 调度器）](#7-sgnlevel-子模块level-调度器)
8. [完整示例：6 层 CNN 前向+反向](#8-完整示例6-层-cnn-前向反向)
9. [双模式一致性测试](#9-双模式一致性测试)
10. [与 PyTorch 互操作](#10-与-pytorch-互操作)
11. [已知限制](#11-已知限制)
12. [故障排查](#12-故障排查)
13. [性能基线与优化（已迁移至性能白皮书）](#13-性能基线与优化已迁移至性能白皮书)
14. [已知 Bug 修复记录](#14-已知-bug-修复记录)
15. [相关文档](#15-相关文档)
16. [诊断与工具](#16-诊断与工具)
17. [Loss 模块（v0.8.0 新增）](#17-loss-模块v080-新增)
- [附录：文件结构](#附录文件结构)
...
- ✅ **测试验证**：全量 pytest **384 项全通过**（2026-09-07 复测；详见 [SGN 性能白皮书](SGN_性能白皮书.md) §3.7）
- ⚠️ **性能公平口径（2026-09-07 重构）**：同任务三方对比（MLP fwd+bwd B=256）：numpy 手写 2.34ms / **SGN 4.77ms** / PyTorch(MKL) 1.56ms——SGN **不主张** float 训练速度超越 PyTorch（vendor BLAS 架构差异）；AVX2 FMA + OpenMP + AVX-VNNI（CPUID 自动启用）见下条。SGN 差异化在整数/量化路径与 LeveledState 平面推理（状态带宽 4.00–4.57×）。详见 [SGN 性能白皮书](SGN_性能白皮书.md) §3.7 与 bench_fair_compare.py
...
- ✅ 标准层 Module 子类（Conv2d/Linear/ReLU/MaxPool2d/BatchNorm2d/Sequential/**Dropout**）：已实现（`nn_layers.py`）
- ✅ **激活族 C++ 算子**（2026-09-07）：sigmoid/tanh/gelu/silu（autograd-aware，`ops_activation.cpp` 独立成族）
- ✅ **mul 逐元素乘算子**（2026-09-07）：Dropout/掩码/缩放地基（autograd-aware）
- ✅ **AdamW 优化器**（2026-09-07）：解耦权重衰减（wd 不污染 m/v 统计）
...
# 优化器（基础设施 B1，2026-08-29）：支持 nn.Module 或 dict[str,ndarray]
...
for epoch in range(num_epochs):
    for x_np, y_np in dataloader:
        ...
        # 更新参数：从 tape 收集梯度后显式传入 step(grads)
...
> ⚠️ **注意（BN 推理限制）**：`model.eval()` 已支持训练/推理模式切换，但**当前 BatchNorm 的 eval（推理）前向尚未实现**（见 [11.1 功能限制](#111-功能限制)）——标准层 `BatchNorm2d` 的前向仍按训练模式计算并更新 running 统计，`model.eval()` 不会自动改用 running 统计，推理时需手动处理 BN。模型不含 BatchNorm 时不受影响。
> 推理时**手动处理 BN** 的三步归一化与可运行示例，见 [SGN 用户使用手册 · 附录：BatchNorm 推理临时处理](SGN_用户使用手册.md#附录batchnorm-推理临时处理)。
...
> **完整训练检查点（基础设施 B2，2026-08-29）**：训练中断续跑需同时保存优化器状态与
> 随机源，用 `sgn.save_checkpoint(path, params=model.state_dict(), optimizer=opt, step=t,
> seed=s, rngs={...})` / `sgn.load_checkpoint(path)`——断点续训与不间断训练逐位一致
> （含 rng 恢复），替代手写 np.savez。
...
| Python 回退 | 活跃层统一使用 C++ `sgn.level`（无 Python 回退） | ✅ 已完成（C++ 化） |
...
> **2026-08-16 更新**：HC8 活跃实现现为 `engine/sgn/hc/` 的 C 扩展（`sgn.hc8_net`）与 `tests/refs/` 的参考实现。本节集成建议为历史参考，待需要时基于活跃实现重新评估。
...
# 更新参数：从 tape 收集梯度后显式 step（基础设施 B1）
grads = {name: p.grad for name, p in model.named_parameters()}
optimizer.step(grads)
```
