...
class JSONStorageBackend(StorageBackend):
    ...
    def serialize(self, core) -> str:
        ...
        data = {
            ...
            "input_dim": getattr(core, "D", CONFIG.get("D", 16)),
            ...
            for layer_id, pool in core.neuron_layers.items():
                ...
                for n in pool:
                    neuron_state = {
                        ...
                        "enc_b": n["enc_b"].serialize() if hasattr(n["enc_b"], "serialize") else n["enc_b"],
                        "L": n["L"],
                        "gate": n["gate"].serialize() if hasattr(n.get("gate"), "serialize") else n.get("gate", {"level": 2, "index": 100}),
        ...
        # 【v4.4 双图叠加】序列化新增字段
        if hasattr(core, "atom_dict") and core.atom_dict:
            data["atom_dict"] = {}
            for label, atom_counts in core.atom_dict.items():
                data["atom_dict"][label] = {str(k): v for k, v in atom_counts.items()}

        if hasattr(core, "high_templates") and core.high_templates:
            data["high_templates"] = core.high_templates

        if hasattr(core, "low_templates") and core.low_templates:
            data["low_templates"] = core.low_templates

        return json.dumps(data, indent=2, ensure_ascii=False)

    def deserialize(self, data: str, core) -> bool:
        """从 JSON 字符串恢复引擎状态"""
        from sgn_config import CONFIG, DiscreteCoordinate
            ...
            for layer_id_str, pool_data in obj["neuron_layers"].items():
                ...
                for n_data in pool_data:
                    ...
                    if isinstance(enc_b_val, dict) and "index" in enc_b_val and "level" in enc_b_val:
                        ...
                        enc_b_dc = DiscreteCoordinate(0, base_dc.level)
                    gate_val = n_data.get("gate", {"level": 2, "index": 100})
                    if isinstance(gate_val, dict) and "index" in gate_val and "level" in gate_val:
                        gate_dc = DiscreteCoordinate.deserialize(gate_val)
                    else:
                        gate_dc = DiscreteCoordinate(100, 2)
                    n = {
        ...
        if "graphs_full" in obj and (graph_mode or getattr(core, 'multi_layer_enabled', False)):
            from sgn_graph import DynamicGraph
            core.graphs = {}
        ...
        if "atom_dict" in obj:
            core.atom_dict = {}
            for label, atom_counts in obj["atom_dict"].items():
                # JSON key 是 str，需转回 int
                core.atom_dict[label] = {int(k): v for k, v in atom_counts.items()}
        else:
            core.atom_dict = {}

        # 高层模板（边缘特征）
        if "high_templates" in obj:
            core.high_templates = obj["high_templates"]
        else:
            core.high_templates = {}

        # 低层模板（像素特征）
        if "low_templates" in obj:
            core.low_templates = obj["low_templates"]
        else:
            core.low_templates = {}

        # 初始化新增成员（如果 core 还没有）
        if not hasattr(core, "hist_buffer"):
            core.hist_buffer = []
        if not hasattr(core, "use_gate_matching"):
            core.use_gate_matching = CONFIG.get("ENABLE_GATE_MATCHING", False)
        if not hasattr(core, "patch_size"):
            core.patch_size = CONFIG.get("PATCH_SIZE", 2)
