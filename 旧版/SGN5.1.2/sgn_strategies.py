...
"""SGN-Lite v5.0 核心策略抽象 —— 解耦 4x4 硬编码阈值

LayerStrategy: 控制二值化层提取逻辑（mark_count, layer_max, min_marked）
VerifyStrategy: 控制校验通过条件（max_span, intensity_diff_thresh）

策略包按窗口尺寸匹配：
  DefaultLayerStrategy    → 兼容 4x4 实验参数，随 D 自适应
  SparseLineStrategy      → 细线条/矢量图形（8x8+），降低标记阈值
"""
...
class DefaultLayerStrategy(LayerStrategy):
    ...
    @property
    def applicable_range(self) -> Tuple[int, int]:
        return (4, 63)  # 4x4 到 8x8，64 归 SparseLineStrategy
    ...
    def get_min_marked(self, d: int) -> int:
        # 原逻辑: max(MIN_MARKED_CNT, d//8)
        return max(CONFIG.get("MIN_MARKED_CNT", 2), d // 8)
...
# ============================================================
# GateMatchStrategy —— 门控匹配策略（jnn.md v4.0）
# ============================================================
...
class MatchStrategy(ABC):
    ...
    控制神经元与输入的匹配方式：
      ...
      - GateMatchStrategy: 门控匹配（高层 AND 低层）
    """
...
class GlobalMatchStrategy(MatchStrategy):
    ...
    def compute(self, n, layers: List[int], layer_count: int, d: int) -> int:
        from sgn_layers import popcount
        t = sum(
...
class GateMatchStrategy(MatchStrategy):
    """门控匹配策略（双图叠加，高层 AND 低层）
    ...
    def __init__(self):
        self.high_thresh = CONFIG.get("GATE_HIGH_THRESH", 60)
        self.low_thresh = CONFIG.get("GATE_LOW_THRESH", 50)

    def compute(self, n, layers: List[int], layer_count: int, d: int) -> int:
        from sgn_layers import extract_edge_map, gate_match
        ...
        # 对于门控匹配，需要原始强度图
        # 这里简化处理：用掩码作为低层特征，用边缘提取作为高层特征
        # 完整实现需要在 train() 中传递原始强度图
        high_in = extract_edge_map([int((input_mask >> i) & 1) * 255 for i in range(d)], d)
        low_in = input_mask

        # 与模板的高层/低层特征匹配
        # 模板的高层/低层特征需要从 core.high_templates/low_templates 获取
        # 这里简化处理：用传统匹配度作为 fallback
        from sgn_layers import popcount
        t = sum(
            d - popcount(n["T"][ll] ^ layers[ll])
            for ll in range(min(layer_count, CONFIG.get("LAYER_MAX", 4)))
